...
static struct hisi_boost5v_control_info *g_boost5v_info;
...
static int regulator_changed(void)
{
	...
	if (g_boost5v_info->flash_led_flag || g_boost5v_info->classd_flag || g_boost5v_info->hdmi_flag || g_boost5v_info->headphone_flag) {
		set_boost5v_voltage_5_v();
		set_boost5v_enable();
		mdelay(3);
	} else if (g_boost5v_info->denoise_headphone_flag) {
