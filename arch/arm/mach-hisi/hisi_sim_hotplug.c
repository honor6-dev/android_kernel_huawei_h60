...
static void set_sim_status(struct hisi_sim_hotplug_info *info, u32 sim_no, u32 sim_status, u32 trace, u32 notify)
{
	...
	pr_info("info->sim_id:%d, sim_mux[0]:%d,sim_mux[1]:%d,sim_mux[2]:%d\n",
		info->sim_id, log_temp.sim_mux[0], log_temp.sim_mux[1], log_temp.sim_mux[2]);
}
...
static void inquiry_sim_det_irq_reg(struct work_struct *work)
{
	...
	pr_info("%s, %d, det_gpio_state:%d, sim_pluged:%d, g_send_msg_to_lpm3:%d\n",
		__func__, __LINE__, det_gpio_state, sim_hotplug_info->sim_pluged, g_send_msg_to_lpm3);
	...
	if (!det_gpio_state) {
		if (sim_hotplug_info->sim_pluged == SIM_LEAVE_POSITION) {
			...
			pr_info("%s, %d, sim_hotplug_info->sim_id=%d, SIM_IN_POSITION \n",
					__func__, __LINE__, sim_hotplug_info->sim_id);
#ifdef CONFIG_HISI_RPROC
			if (g_send_msg_to_lpm3) {
		...
		if (sim_hotplug_info->sim_pluged == SIM_IN_POSITION) {
			...
			pr_info("%s, %d, sim_hotplug_info->sim_id=%d, SIM_LEAVE_POSITION \n",
					__func__, __LINE__, sim_hotplug_info->sim_id);
#ifdef CONFIG_HISI_RPROC
			if (g_send_msg_to_lpm3) {
...
static int sim_hotplug_dt_init(struct hisi_sim_hotplug_info *info, struct device_node *np, struct device *dev)
{
	...
	ret = of_property_read_u32(np, "send_msg_to_lpm3", &g_send_msg_to_lpm3);
	if (ret < 0) {
