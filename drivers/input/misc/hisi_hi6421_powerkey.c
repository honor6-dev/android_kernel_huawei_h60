	...
	if (powerkey_press_count > MAX_PRESS_KEY_COUNT) {
		if(!dsm_client_ocuppy(power_key_dclient)) {
			dsm_client_record(power_key_dclient,
				"powerkey was triggered more than %d times within %d seconds.\n",
...
static irqreturn_t hi6421_powerkey_handler(int irq, void *data)
{
	...
	if (irq == info->irq[0]) {
		...
		if ((jiffies - powerkey_last_press_time) < msecs_to_jiffies(PRESS_KEY_INTERVAL)) {
			if (!dsm_client_ocuppy(power_key_dclient)) {
				dsm_client_record(power_key_dclient,
					"power key was triggered twice within %d milliseconds.\n", PRESS_KEY_INTERVAL);
				dsm_client_notify(power_key_dclient, DSM_POWER_KEY_ERROR_NO);
			}
		}
		...
		if (!dsm_client_ocuppy(power_key_dclient)) {
			dsm_client_record(power_key_dclient,
				"power key long press 8s irq:%d was triggered.\n", irq);
			dsm_client_notify(power_key_dclient, DSM_POWER_KEY_LONG_PRESS_ERROR_NO);
		}
