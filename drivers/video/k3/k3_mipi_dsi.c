	...
	struct mipi_dsi_phy_ctrl *phy_ctrl)
{
	...
	** 5MHz <= Frefclk/N <= 40MHz*/
	...
	if (range <= 110) {
		...
		K3_FB_ERR("unsupport the value of PLL loop divider ratio (M) (%d)!\n", m_pll);
		return;
	}
	...
	if (range <= 90) {
		...
		K3_FB_ERR("unsupport this range(%d)!\n", range);
		return;
	}
...
static void mipi_init(struct k3_fb_data_type *k3fd, char __iomem *mipi_dsi_base)
{
	u32 hline_time = 0;
	u32 hsa_time = 0;
	u32 hbp_time = 0;
	...
	*/
	...
	if (!is_ready) {
		K3_FB_INFO("fb%d, phylock is not ready!MIPIDSI_PHY_STATUS_OFFSET=0x%x.\n",
			k3fd->index, tmp);
	}
	...
	if (!is_ready) {
		K3_FB_INFO("fb%d, phystopstateclklane is not ready! MIPIDSI_PHY_STATUS_OFFSET=0x%x.\n",
			k3fd->index, tmp);
	}
}
...
int mipi_dsi_clk_enable(struct k3_fb_data_type *k3fd)
{
	...
	clk_tmp = k3fd->dss_dphy0_clk;
	if (clk_tmp) {
		ret = clk_prepare(clk_tmp);
		if (ret) {
			K3_FB_ERR("fb%d dss_dphy0_clk clk_prepare failed, error=%d!\n",
				k3fd->index, ret);
			return -EINVAL;
		}
		...
		if (ret) {
			K3_FB_ERR("fb%d dss_dphy0_clk clk_enable failed, error=%d!\n",
				k3fd->index, ret);
			return -EINVAL;
		}
	}
	...
	if (is_dual_mipi_panel(k3fd)) {
		clk_tmp = k3fd->dss_dphy1_clk;
		if (clk_tmp) {
			ret = clk_prepare(clk_tmp);
			if (ret) {
				K3_FB_ERR("fb%d dss_dphy1_clk clk_prepare failed, error=%d!\n",
					k3fd->index, ret);
				return -EINVAL;
			}
			...
			ret = clk_enable(clk_tmp);
			if (ret) {
				K3_FB_ERR("fb%d dss_dphy1_clk clk_enable failed, error=%d!\n",
					k3fd->index, ret);
				return -EINVAL;
			}
		}
...
**
*/
static int mipi_dsi_on_sub1(struct k3_fb_data_type *k3fd, char __iomem *mipi_dsi_base)
{
	BUG_ON(mipi_dsi_base == NULL);

	/* mipi init */
	mipi_init(k3fd, mipi_dsi_base);
...
static int mipi_dsi_on_sub2(struct k3_fb_data_type *k3fd, char __iomem *mipi_dsi_base)
{
	BUG_ON(k3fd == NULL);
	BUG_ON(mipi_dsi_base == NULL);

	if (is_mipi_video_panel(k3fd)) {
...
int mipi_dsi_off_sub(struct k3_fb_data_type *k3fd, char __iomem *mipi_dsi_base)
{
	BUG_ON(k3fd == NULL);
	BUG_ON(mipi_dsi_base == NULL);
...
static int mipi_dsi_ulps_enter(struct k3_fb_data_type *k3fd,
	char __iomem *mipi_dsi_base)
{
	...
	if (k3fd->panel_info.mipi.lane_nums >= DSI_4_LANES) {
		cmp_val = (BIT(5) | BIT(8) | BIT(10) | BIT(12));
	} else if (k3fd->panel_info.mipi.lane_nums >= DSI_3_LANES) {
		cmp_val = (BIT(5) | BIT(8) | BIT(10));
	} else if (k3fd->panel_info.mipi.lane_nums >= DSI_2_LANES) {
		cmp_val = (BIT(5) | BIT(8));
	} else {
		cmp_val = (BIT(5));
	}
	...
	while ((tmp & cmp_val) != 0) {
		udelay(10);
		if (++try_times > 100) {
			K3_FB_ERR("fb%d, check DPHY data lane status failed! MIPIDSI_PHY_STATUS=0x%x.\n",
				k3fd->index, tmp);
			break;
		}
	...
	while ((tmp & BIT(3)) != 0) {
		udelay(10);
		if (++try_times > 100) {
			K3_FB_ERR("fb%d, check DPHY clock lane status failed! MIPIDSI_PHY_STATUS=0x%x.\n",
				k3fd->index, tmp);
			break;
		}
	...
	if (is_dual_mipi_panel(k3fd))
		outp32(k3fd->crgperi_base + PERDIS3_OFFSET, 0x500000);
	else
		outp32(k3fd->crgperi_base + PERDIS3_OFFSET, 0x100000);
...
static int mipi_dsi_ulps_exit(struct k3_fb_data_type *k3fd,
	char __iomem *mipi_dsi_base)
{
	u32 tmp = 0;
	u32 cmp_val = 0;
	...
	if (is_dual_mipi_panel(k3fd))
		outp32(k3fd->crgperi_base + PEREN3_OFFSET, 0x500000);
	else
		outp32(k3fd->crgperi_base + PEREN3_OFFSET, 0x100000);
	...
	while ((tmp & BIT(0)) != 0X1) {
		udelay(10);
		if (++try_times > 100) {
			K3_FB_ERR("fb%d, failed to wait DPHY PLL Lock!MIPIDSI_PHY_STATUS=0x%x.\n",
				k3fd->index, tmp);
			break;
		}
	...
	while ((tmp & cmp_val) != cmp_val) {
		udelay(10);
		if (++try_times > 100) {
			K3_FB_ERR("fb%d, failed to request that data lane and clock lane exit ULPS!MIPIDSI_PHY_STATUS=0x%x.\n",
				k3fd->index, tmp);
			break;
		}
...
int mipi_dsi_ulps_cfg(struct k3_fb_data_type *k3fd, int enable)
{
	int ret = 0;
	...
	mipi_dsi1_base = k3fd->dss_base + DSS_MIPI_DSI1_OFFSET;

	if (enable) {
		mipi_dsi_ulps_exit(k3fd, mipi_dsi0_base);
		if (is_dual_mipi_panel(k3fd))
			mipi_dsi_ulps_exit(k3fd, mipi_dsi1_base);
	} else {
		mipi_dsi_ulps_enter(k3fd, mipi_dsi0_base);
		if (is_dual_mipi_panel(k3fd))
			mipi_dsi_ulps_enter(k3fd, mipi_dsi1_base);
	}
...
void mipi_dsi_bugfix(struct k3_fb_data_type *k3fd)
{
	char __iomem *mipi_dsi0_base = NULL;
	char __iomem *mipi_dsi1_base = NULL;
...
static int mipi_dsi_on(struct platform_device *pdev)
{
	...
	mipi_dsi_on_sub1(k3fd, mipi_dsi0_base);
	if (is_dual_mipi_panel(k3fd))
		mipi_dsi_on_sub1(k3fd, mipi_dsi1_base);

	ret = panel_next_on(pdev);

	mipi_dsi_on_sub2(k3fd, mipi_dsi0_base);
	if (is_dual_mipi_panel(k3fd))
		mipi_dsi_on_sub2(k3fd, mipi_dsi1_base);
...
static int mipi_dsi_off(struct platform_device *pdev)
{
	...
	mipi_dsi_off_sub(k3fd, mipi_dsi0_base);
	if (is_dual_mipi_panel(k3fd))
		mipi_dsi_off_sub(k3fd, mipi_dsi1_base);
...
static int mipi_dsi_remove(struct platform_device *pdev)
{
	...
	if (k3fd->dss_dphy0_clk) {
		clk_put(k3fd->dss_dphy0_clk);
		k3fd->dss_dphy0_clk = NULL;
	}

	if (k3fd->dss_dphy1_clk) {
		clk_put(k3fd->dss_dphy1_clk);
		k3fd->dss_dphy1_clk = NULL;
	}
...
static int mipi_dsi_lcd_gram_checksum(struct platform_device *pdev, unsigned char *buf)
{
	BUG_ON(pdev == NULL);

	return panel_next_lcd_gram_checksum(pdev, buf);
}
#endif
...
static int mipi_dsi_clk_irq_setup(struct platform_device *pdev)
{
	...
	k3fd->dss_dphy0_clk = devm_clk_get(&pdev->dev, k3fd->dss_dphy0_clk_name);
	if (IS_ERR(k3fd->dss_dphy0_clk)) {
		ret = PTR_ERR(k3fd->dss_dphy0_clk);
		return ret;
	} else {
		ret = clk_set_rate(k3fd->dss_dphy0_clk, DEFAULT_MIPI_CLK_RATE);
		if (ret < 0) {
			K3_FB_ERR("fb%d dss_dphy0_clk clk_set_rate(%lu) failed, error=%d!\n",
				k3fd->index, DEFAULT_MIPI_CLK_RATE, ret);
			return -EINVAL;
		}

		K3_FB_INFO("dss_dphy0_clk:[%lu]->[%lu].\n",
			DEFAULT_MIPI_CLK_RATE, clk_get_rate(k3fd->dss_dphy0_clk));
	}

	if (is_dual_mipi_panel(k3fd)) {
		k3fd->dss_dphy1_clk = devm_clk_get(&pdev->dev, k3fd->dss_dphy1_clk_name);
		if (IS_ERR(k3fd->dss_dphy1_clk)) {
			ret = PTR_ERR(k3fd->dss_dphy1_clk);
			return ret;
		} else {
			ret = clk_set_rate(k3fd->dss_dphy1_clk, DEFAULT_MIPI_CLK_RATE);
			if (ret < 0) {
				K3_FB_ERR("fb%d dss_dphy1_clk clk_set_rate(%lu) failed, error=%d!\n",
					k3fd->index, DEFAULT_MIPI_CLK_RATE, ret);
				return -EINVAL;
			}

			K3_FB_INFO("dss_dphy1_clk:[%lu]->[%lu].\n",
				DEFAULT_MIPI_CLK_RATE, clk_get_rate(k3fd->dss_dphy1_clk));
		}
	}
...
static int mipi_dsi_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		K3_FB_ERR("fb%d mipi_dsi_irq_clk_setup failed, error=%d!\n", k3fd->index, ret);
		goto err;
	}
...
#endif
#ifdef CONFIG_LCD_GRAM_CHECKSUM
	pdata->lcd_gram_checksum = mipi_dsi_lcd_gram_checksum;
#endif
	pdata->next = pdev;
	...
	/* set driver data */
	platform_set_drvdata(dpp_dev, k3fd);
	/* device add */
...
*****mipi freq fs
*/
static ssize_t hisi_lcd_mipi_freq(struct device *dev, struct device_attribute *attr,
      const char *buf, size_t count)
{
	size_t n_str = 0;
	struct k3_fb_data_type *k3fd0 = k3fd_list[PRIMARY_PANEL_IDX];
	struct k3_panel_info *pinfo = NULL;

	if(!k3fd0 || !k3fd0->panel_info.mipi_emi_support) {
		return count;
	}

	pinfo = &(k3fd0->panel_info);
	BUG_ON(pinfo == NULL);

	n_str = count - 1;

	if (!strncmp(buf, MIPI_DSI_FREQ1_STR, n_str)) {
		pinfo->mipi.dsi_bit_clk_next= pinfo->mipi.mipi_dsi_freq1;
	} else if (!strncmp(buf, MIPI_DSI_FREQ2_STR, n_str)) {
		pinfo->mipi.dsi_bit_clk_next= pinfo->mipi.mipi_dsi_freq2;
	} else if (!strncmp(buf, MIPI_DSI_FREQ3_STR, n_str)) {
		pinfo->mipi.dsi_bit_clk_next= pinfo->mipi.mipi_dsi_freq3;
	} else if (!strncmp(buf, MIPI_DSI_FREQ4_STR, n_str)) {
		pinfo->mipi.dsi_bit_clk_next= pinfo->mipi.mipi_dsi_freq4;
	} else if (!strncmp(buf, MIPI_DSI_FREQ5_STR, n_str)) {
		pinfo->mipi.dsi_bit_clk_next= pinfo->mipi.mipi_dsi_freq5;
	} else {
		K3_FB_ERR("unknown mipi frequency index!!\n");
	}

	return count;
}

static DEVICE_ATTR(lcd_mipi_freq, S_IWUSR, NULL, hisi_lcd_mipi_freq);
static struct attribute *lcd_mipi_freq_fs_attrs[] = {
	&dev_attr_lcd_mipi_freq.attr,
	NULL,
};

static struct attribute_group lcd_mipi_freq_fs_attr_group = {
	.attrs = lcd_mipi_freq_fs_attrs,
};

void k3fb_mipi_freq_register(struct platform_device *pdev)
{
	int ret = 0;
	struct k3_fb_data_type *k3fd = NULL;
	...
	ret = sysfs_create_group(&k3fd->fbi->dev->kobj, &lcd_mipi_freq_fs_attr_group);
	if (ret) {
		K3_FB_ERR("fb%d lcd mipi freq sysfs group creation failed, error=%d!\n",
			k3fd->index, ret);
	}
}

void k3fb_mipi_freq_unregister(struct platform_device *pdev)
{
	struct k3_fb_data_type *k3fd = NULL;
	...
	k3fd = platform_get_drvdata(pdev);
	BUG_ON(k3fd == NULL);
	sysfs_remove_group(&k3fd->fbi->dev->kobj, &lcd_mipi_freq_fs_attr_group);
}

int mipi_freq_update(struct k3_fb_data_type *k3fd)
{
	u32 hline_time = 0;
	u32 hsa_time = 0;
	u32 hbp_time = 0;
	...
	/* if mipi clock dsi_bit_clk_next is the same as current dsi_bit_clk, return! */
	if (pinfo->mipi.dsi_bit_clk_next == pinfo->mipi.dsi_bit_clk)
		return 0;

	K3_FB_DEBUG("fb%d +.\n", k3fd->index);

	/* get new phy_ctrl value according to dsi_bit_clk_next */
	get_dsi_phy_ctrl(pinfo->mipi.dsi_bit_clk_next, &phy_ctrl);
	...
	*/
	...
	if (!is_ready) {
		K3_FB_INFO("fb%d, phystopstatedatalane is not ready! MIPIDSI_PHY_STATUS_OFFSET=0x%x.\n",
			k3fd->index, tmp);
	...
	pinfo->mipi.dsi_bit_clk = pinfo->mipi.dsi_bit_clk_next;
