...
static int k3_fb_gram_checksum(struct fb_info *info, unsigned char *buf)
{
	...
	if (!k3fd->panel_power_on) {
		K3_FB_ERR("system is now power off !\n");
		ret = LCD_GRAM_CHECKSUM_FAIL;
		goto gram_checksum_error;
	}

	pdata = dev_get_platdata(&k3fd->pdev->dev);
	BUG_ON(pdata == NULL);

	if ((pdata) && (pdata->lcd_gram_checksum)) {
		k3fb_activate_vsync(k3fd);
		ret = pdata->lcd_gram_checksum(k3fd->pdev, buf);
		k3fb_deactivate_vsync(k3fd);
	} else {
		K3_FB_ERR("This panel can not support gram checksum function");
		ret = LCD_GRAM_CHECKSUM_FAIL;
	}
	...
	struct fb_info *info)
{
	...
	if (!info->screen_base) {
               K3_FB_ERR("fb%d, not alloc framebuffer!\n", k3fd->index);
               return -ENOMEM;
	}
...
static int k3_fb_ioctl(struct fb_info *info, unsigned int cmd, unsigned long arg)
{
...
#ifdef CONFIG_LCD_GRAM_CHECKSUM
	int rc = LCD_GRAM_CHECKSUM_FAIL;
	unsigned char gram_check_sum_buf[8];
#endif
	BUG_ON(info == NULL);
	k3fd = (struct k3_fb_data_type *)info->par;
	BUG_ON(k3fd == NULL);
	...
	//Multi Resolution====end
#ifdef CONFIG_LCD_GRAM_CHECKSUM
	case K3FB_GRAM_CHECKSUM:
		if (copy_from_user(gram_check_sum_buf, argp, 8)){
			K3_FB_ERR("copy from user failed\n");
			return LCD_GRAM_CHECKSUM_FAIL;
		}
		rc = k3_fb_gram_checksum(info, gram_check_sum_buf);
		ret = copy_to_user(argp, &gram_check_sum_buf, sizeof(gram_check_sum_buf));
		if (rc || ret) {
			printk(KERN_ERR "%s: K3FB_GRAM_CHECKSUM ioctl failed \n", __func__);
			return rc;
		}
		break;
#endif
...
static int k3_fb_mmap(struct fb_info *info, struct vm_area_struct * vma)
{
	...
	if (k3fd->index == PRIMARY_PANEL_IDX) {
		if (k3fd->fb_mem_free_flag) {
			if (!k3fb_alloc_fb_buffer(k3fd)) {
				K3_FB_ERR("fb%d, hisifb_alloc_buffer failed!\n", k3fd->index);
				return -ENOMEM;
			}
		}
	} else {
...
unsigned long k3fb_alloc_fb_buffer(struct k3_fb_data_type *k3fd)
{
	struct fb_info *fbi = NULL;
...
void k3fb_free_fb_buffer(struct k3_fb_data_type *k3fd)
{
	struct fb_info *fbi = NULL;
...
static int k3_fb_register(struct k3_fb_data_type *k3fd)
{
	...
	k3fd->fb_mem_free_flag = false;
	k3fd->ref_cnt = 0;
	k3fd->panel_power_on = false;
	k3fd->blank_in_suspend = false;
	...
	if (k3fd->index == PRIMARY_PANEL_IDX) {
		...
		k3fd->mipi_freq_update = mipi_freq_update;
		k3fd->mipi_freq_register = k3fb_mipi_freq_register;
		k3fd->mipi_freq_unregister = k3fb_mipi_freq_unregister;
	} else if (k3fd->index == EXTERNAL_PANEL_IDX) {
		k3fd->set_fastboot_fnc = NULL;
		...
		k3fd->mipi_freq_update = NULL;
		k3fd->mipi_freq_register = NULL;
		k3fd->mipi_freq_unregister = NULL;
	...
	if (k3fd->mipi_freq_register)
		k3fd->mipi_freq_register(k3fd->pdev);
...
static int k3_fb_remove(struct platform_device *pdev)
{
	...
	if (k3fd->mipi_freq_unregister)
		k3fd->mipi_freq_unregister(k3fd->pdev);
...
static int k3_fb_pm_suspend(struct device *dev)
{
	struct k3_fb_data_type *k3fd = NULL;
	...
	k3fd = dev_get_drvdata(dev);
	if (!k3fd || (!k3fd->panel_power_on))
		return 0;

	K3_FB_INFO("fb%d, +.\n", k3fd->index);

	info = registered_fb[k3fd->index];
	if (!info || k3fd->index)
		return 0;

	k3fd->blank_in_suspend = true;
...
static int k3_fb_pm_resume(struct device *dev)
{
	struct k3_fb_data_type *k3fd = NULL;
	struct fb_info *info= NULL;
	int ret = 0;
...
static struct dev_pm_ops k3_fb_dev_pm_ops = {
	...
	.suspend = k3_fb_pm_suspend,
	.resume = k3_fb_pm_resume,
};
