...
static int mipi_jdi_panel_on(struct platform_device *pdev)
{
	...
	u32 pkg_status = 0, power_status = 0, try_times = 0, pwm_status_value = 0;
	...
	if (pinfo->lcd_init_step == LCD_INIT_POWER_ON) {
		...
		if (likely(!fastboot_display_enable)) {
			...
			if ((0x9c != power_status) && (dsm_error_found >= 0)) {
				dsm_client_record(lcd_dclient, "LCD Power State = 0x%x\n", power_status);
				dsm_error_found++;
			}

			outp32(mipi_dsi0_base + MIPIDSI_GEN_HDR_OFFSET, 0xAC06);
			pkg_status = inp32(mipi_dsi0_base + MIPIDSI_CMD_PKT_STATUS_OFFSET);
			while (pkg_status & 0x10) {
				udelay(50);
				if (++try_times > 100) {
					try_times = 0;
					K3_FB_ERR("Read lcd PWM status timeout\n");
					break;
				}
				pkg_status = inp32(mipi_dsi0_base + MIPIDSI_CMD_PKT_STATUS_OFFSET);
			}

			pwm_status_value = inp32(mipi_dsi0_base + MIPIDSI_GEN_PLD_DATA_OFFSET);
			if ((0x00 != pwm_status_value) && (dsm_error_found >= 0)) {
				dsm_client_record(lcd_dclient, "LCD PWM State = 0x%x\n", pwm_status_value);
				dsm_error_found++;
			}
