...
#define DTS_COMP_JDI_NT35695B "hisilicon,mipi_jdi_NT35695B"
...
unsigned char get_mipi_jdi_NT35695B_tp_color(void)
{
	u32 out = 0;
	u8 lcd_chip_id = 0xFF;
	char __iomem *mipi_dsi0_base = NULL;
...
static int mipi_jdi_probe(struct platform_device *pdev)
{
	...
	np = of_find_compatible_node(NULL, NULL, DTS_COMP_JDI_NT35695B);
	if (!np) {
		K3_FB_ERR("NOT FOUND device node %s!\n", DTS_COMP_JDI_NT35695B);
		goto err_return;
	}
	...
	get_tp_color = get_mipi_jdi_NT35695B_tp_color;
...
static const struct of_device_id k3_panel_match_table[] = {
	{
	.compatible = DTS_COMP_JDI_NT35695B,
	.data = NULL,
	},
};
...
static int __init mipi_jdi_panel_init(void)
{
	...
	if (read_lcd_type() != JDI_NT35695B_LCD) {
		K3_FB_INFO("lcd type is not jdi_NT35695B_LCD, return!\n");
		return ret;
	}
