...
*/
void ov_cmd_list_output_32(struct k3_fb_data_type *k3fd, char __iomem *base_addr,
	u32 value, u8 bw, u8 bs)
{
	int index;
	u32 item_addr;
	u32 mask = (1 << bw) - 1;
	...
	wb_chn = k3fd->ov_req.wb_layer_info.chn_idx - WBE1_CHN0;
	if (wb_chn  >= K3_DSS_OFFLINE_MAX_NUM ) {
		K3_FB_ERR("not support, wbe_chn=%d!\n", wb_chn);
		return;
	}

	/*get the last list*/
	cmdlist_node = list_entry(k3fd->offline_cmdlist_head[wb_chn].prev, dss_cmdlist_node_t, list_node);
	//K3_FB_ERR("list header_addr:0x%p\n", cmdlist_node->list_header);
	BUG_ON(cmdlist_node == NULL);
	if (cmdlist_node->list_node_type== e_node_nop) {
		K3_FB_ERR("can't set register value to NOP node!\n");
		return;
	}

	index = cmdlist_node->item_index;
	item_addr = (u32) base_addr >> 2;
	item_addr = item_addr & CMDLIST_ADDR_OFFSET;

	do {
		u32 old_addr= cmdlist_node->list_item[index].reg_addr.ul32 & CMDLIST_ADDR_OFFSET;
		u32 offset = item_addr - old_addr;
		...
		if (bw != 32) {
			...
			//K3_FB_ERR("should not set %d bits,data:0x%x addr:0x%x \n", bw, value, (u32)base_addr);
			break;
		}

		if (cmdlist_node->item_flag == 0) {
			//K3_FB_ERR("%d set %d bits,old_addr:0x%x item_addr:0x%x offset:%d \n", cmdlist_node->item_flag, bw, old_addr, (u32)item_addr, offset);
			cmdlist_node->list_item[index].reg_addr.bits.add0 = item_addr;
			cmdlist_node->list_item[index].data0 = value;
...
int cmdlist_alloc_one_list(dss_cmdlist_node_t ** cmdlist_node, struct ion_client *ion_client)
{
	...
	cmdlist->header_ion_handle = ion_alloc(ion_client, header_len, 0, ION_HEAP(ION_MISC_HEAP_ID), 0);
	...
	cmdlist->item_ion_handle = ion_alloc(ion_client, item_len, 0, ION_HEAP(ION_MISC_HEAP_ID), 0);
...
dss_cmdlist_node_t * cmdlist_get_one_list(struct k3_fb_data_type *k3fd, int * id)
{
	...
	for(i = 0; i < K3_DSS_OFFLINE_MAX_LIST; i++){
		if(k3fd->cmdlist_node[i]->is_used == FALSE){
			k3fd->cmdlist_node[i]->is_used = TRUE;
			*id = i + 1;
			return k3fd->cmdlist_node[i];
		}
	}
...
int cmdlist_add_nop_list(struct k3_fb_data_type *k3fd, struct list_head *cmdlist_head, int is_first, int is_last)
{
	dss_cmdlist_node_t * cmdlist_node = NULL;
	int ret = 0;
	int id = 0;
	...
	cmdlist_node = cmdlist_get_one_list(k3fd, &id);
...
int cmdlist_add_new_list(struct k3_fb_data_type *k3fd, struct list_head * cmdlist_head, int is_last, u32 flag)
{
	...
	if (cmdlist_node->list_node.prev != cmdlist_head) {
		...
		pre_node = list_entry(cmdlist_node->list_node.prev, dss_cmdlist_node_t, list_node);
		pre_node->list_header->next_list = cmdlist_node->header_phys;
		//K3_FB_ERR("is_last:%d next_list = 0x%x\n", is_last, (u32)header_phys);
	}
...
int cmdlist_prepare_next_frame(struct k3_fb_data_type *k3fd, u32 wbe_chn)
{
	...
	if (check_if_have_another_frame(&k3fd->offline_cmdlist_head[wbe_chn])) {
...
{
	char __iomem *cmd_list_base = k3fd->dss_base + DSS_CMD_LIST_OFFSET;
	u32 offset = CMDLIST_CH5_CTRL - CMDLIST_CH4_CTRL;
	u32 status = 0;

	BUG_ON(k3fd == NULL);
	BUG_ON(wbe_chn >= K3_DSS_OFFLINE_MAX_NUM);

	if (k3fd->offline_wb_status[wbe_chn] == e_status_wait) {
...
int cmdlist_config_start(struct k3_fb_data_type *k3fd, u32 wbe_chn)
{
	...
	cmdlist_add_nop_list(k3fd, &k3fd->offline_cmdlist_head[wbe_chn], TRUE, FALSE);
	cmdlist_add_nop_list(k3fd, &k3fd->offline_cmdlist_head[wbe_chn], FALSE, FALSE);

	cmdlist_node = list_first_entry(&k3fd->offline_cmdlist_head[wbe_chn], dss_cmdlist_node_t, list_node);
	...
	status = inp32(cmd_list_base + CMDLIST_CH4_STATUS + wbe_chn * offset);
	if((status & 0x07) != 0x07){
		printk(KERN_ERR "cmdlist_config_start fail status:0x%x \n",status);
	}
	return 0;
}
...
int cmdlist_config_stop(struct k3_fb_data_type *k3fd, u32 wbe_chn)
{
	...
	if(count >= 50)
		printk("cmdlist_config_stop errr, can not exit to idle!\n");

	set_reg(cmd_list_base + CMDLIST_SWRST, 1, 1, 4 + wbe_chn);
	set_reg(cmd_list_base + CMDLIST_CH4_CTRL + wbe_chn * offset, 0x0, 2, 0);

	cmdlist_free_all(&k3fd->offline_cmdlist_head[wbe_chn], k3fd->ion_client);
...
void cmdlist_print_node_items(cmd_item_t * item, u32 count)
{
	...
	for (index = 0; index < count; index++) {
		...
		addr = addr << 2;
		K3_FB_INFO("set addr:0x%x value:0x%x add1:0x%x value:0x%x add2:0x%x value:0x%x \n",
...
void cmdlist_print_one_node(dss_cmdlist_node_t * node)
{
	...
	if (node->list_node_type == e_node_first_nop) {
		K3_FB_INFO("node type = fisrt NOP node\n");
	} else if (node->list_node_type == e_node_nop ) {
		K3_FB_INFO("node type = NOP node\n");
	} else {
		K3_FB_INFO("node type = Frame node, flag = 0x%x\n", node->list_node_flag);
	}

	K3_FB_INFO("\t flag | pending | list_addr  | next_list  | count      |  id        \n");
	K3_FB_INFO("\t------+---------+------------+------------+------------+------------\n");
	K3_FB_INFO("\t 0x%2x | 0x%5x | 0x%8x | 0x%8x | 0x%8x | 0x%8x \n\n",
...
void cmdlist_print_all_node(struct list_head * cmdlist_head)
{
	...
	list_for_each_entry_safe(node, __node__, cmdlist_head, list_node) {
		//u32 count = node->list_header->total_items.bits.count;
