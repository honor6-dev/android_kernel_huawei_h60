	...
	if (k3fd->index == PRIMARY_PANEL_IDX) {
		...
		*/
		...
		if (is_mipi_cmd_panel(k3fd))
			unmask &= ~(BIT_BACKLIGHT_INT | BIT_VACTIVE0_START_INT| BIT_VACTIVE0_END_INT | BIT_LDI_TE0_PIN_INT);
		else
			unmask &= ~(BIT_BACKLIGHT_INT | BIT_VSYNC_INT | BIT_VACTIVE0_START_INT| BIT_VACTIVE0_END_INT);
...
static int dpe_lcd_gram_checksum(struct platform_device *pdev, unsigned char *buf)
{
	BUG_ON(pdev == NULL);
...
#endif
#ifdef CONFIG_LCD_GRAM_CHECKSUM
	pdata->lcd_gram_checksum = dpe_lcd_gram_checksum;
#endif
	pdata->next = pdev;
