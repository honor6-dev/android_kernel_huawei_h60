...
#define MIN_CHANNEL 1 //channel 0 is reserved for M3 processor

/* Milliseconds */
#define K3DMA_AUTOSUSPEND_DELAY	1000
...
static void k3_dma_pause_dma(struct k3_dma_phy *phy, bool on)
{
	...
	if (on) {
		...
		for (timeout = 2000; timeout > 0; timeout--) {
			if (!readl_relaxed(phy->base + CH_STAT))
				break;
			udelay(1);
		}
...
static void k3_dma_set_desc(struct k3_dma_phy *phy, struct k3_desc_hw *hw)
{
	...
	writel_relaxed(axi_cfg, phy->base + AXI_CONFIG);
...
static void k3_dma_enable_dma(struct k3_dma_dev *d, bool on)
{
	if (on) {
		...
		/* unmask irq */
		writel_relaxed(0xfffe, d->base + INT_TC1_MASK);
		writel_relaxed(0xfffe, d->base + INT_ERR1_MASK);
		writel_relaxed(0xfffe, d->base + INT_ERR2_MASK);
	} else {
		/* mask irq */
...
#if 0
static void print_desc(struct k3_dma_phy *phy)
{
...
static irqreturn_t k3_dma_int_handler(int irq, void *dev_id)
{
	...
	writel_relaxed(tc1_irq, d->base + INT_TC1_RAW);
	writel_relaxed(err1_irq, d->base + INT_ERR1_RAW);
	writel_relaxed(err2_irq, d->base + INT_ERR2_RAW);
...
static void k3_dma_tasklet(unsigned long arg)
{
	...
	for (pch = MIN_CHANNEL; pch < d->dma_channels; pch++) {
		p = &d->phy[pch];
...
void show_dma_reg(struct dma_chan *chan)
{
        ...
        if (c->phy->base) {
	        dev_info(d->slave.dev, "d->phy->base = %p \n", c->phy->base);
        }
        else {
	        dev_info(d->slave.dev, "d->phy->base is NULL!");
                return;
        }
...
static int k3_dma_probe(struct platform_device *op)
{
	...
	for (i = MIN_CHANNEL; i < d->dma_channels; i++) {
	...
	pm_runtime_set_autosuspend_delay(d->slave.dev, K3DMA_AUTOSUSPEND_DELAY);
...
#endif
	.suspend_late = k3_dma_pltfm_suspend,
	.resume_early	= k3_dma_pltfm_resume,
};
...
static int __init dmac_module_init(void)
{
	...
	retval = platform_driver_register(&k3_pdma_driver);
	if (retval) {
...
{
	platform_driver_unregister(&k3_pdma_driver);
}
