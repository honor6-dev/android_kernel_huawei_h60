...
static struct dsm_dev dsm_dev_bt = {
       .name = "dsm_bluetooth",
       .fops = NULL,
       .buff_size = 1024,
};

static struct dsm_client *dsm_client_bt = NULL;
...
static int bluetooth_power_probe(struct platform_device *pdev){
	...
	if (!dsm_client_bt)
	{
		dsm_client_bt = dsm_register_client(&dsm_dev_bt);
		if (!dsm_client_bt)
			printk(KERN_INFO "bluetooth dsm dev register failed\n");
	}
