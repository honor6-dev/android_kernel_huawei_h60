...
//#define VPP_RESERVED_MEM_ADDR  0x3FA00000
#define VPP_RESERVED_BASE_SIZE 5*1024*1024

//add for mmu
#define VPP_MMU_RESERVED_MEM_ADDR  0x32000000
#define VPP_MMU_VIR_MEM_ADDR  0x38000000
#define VPP_MMU_RESERVED_BASE_SIZE 100*1024*1024
#define VPP_MMU_RESERVED_PAGE_SIZE 4*1024
#define VPP_MMU_RESERVED_PAGE_NUM  20*1024
#define VPP_MMU_ADDR_EXCURSION
#define OFFSET 0x0000000

#define DIE_FRAME_MAX_WIDTH (1920)
#define DIE_FRAME_MAX_HEIGHT (1088)
...
static int alloc_vpp_reserved_mem(struct k3_vpp_addr *k3vpp, unsigned int buf_size){
    ...
    logi("in alloc_vpp_reserved_mem\n");
    if (NULL == client){
        loge("failed to create ion client!\n");
        goto err_ion_client_create;
    }
    handle = ion_alloc(client, buf_size, PAGE_SIZE, ION_HEAP(ION_MISC_HEAP_ID), 0);
    ...
    logi("vpp----phys_addr=%ld",phys_addr);
    return phys_addr;
err_ion_phys:
...
static int inter_init_dieinfo(VPP_DIE_ADDR_S *pstDieModule)
{
    ...
    logi("inter_init_dieinfo --phy_addr=%0x\n",phy_addr);
    pstDieModule->die_VirtAddr
            = (unsigned int*)ioremap(phy_addr, VPP_RESERVED_BASE_SIZE);
...
static void inter_deinit_dieinfo(VPP_DIE_ADDR_S *pstDieModule)
{
    ...
    {
        iounmap(pstDieModule->die_VirtAddr);
...
int inter_start(VPP_CONFIG_S* pVppConfig)
{
    ...
    if(1 == pVppConfig->bdie_enbale)
    {
        ...
        hal_set_field_order(HAL_LAYER_VIDEO1, pVppConfig->edie_fod);
        /*if(bTop == 1)
        {
