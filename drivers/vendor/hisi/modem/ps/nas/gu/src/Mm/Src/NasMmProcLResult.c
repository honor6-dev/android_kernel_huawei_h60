...
)
{
    ...
    */
    ...
    if (VOS_TRUE == pstLmmCombinedAttachInd->bitOpLai)
    {
        ...
        /* 保存到MM old lai全局变量中 */
        g_MmGlobalInfo.MsCsInfo.OldLai.PlmnId.ulMcc = pstCsSuccLai->stPlmnId.ulMcc;
        g_MmGlobalInfo.MsCsInfo.OldLai.PlmnId.ulMnc = pstCsSuccLai->stPlmnId.ulMnc;

        g_MmGlobalInfo.MsCsInfo.OldLai.ulLac = (pstCsSuccLai->aucLac[0] << 8) & 0xFF00;
        g_MmGlobalInfo.MsCsInfo.OldLai.ulLac |= pstCsSuccLai->aucLac[1];
    }
...
)
{
    ...
    NAS_MML_PLMN_ID_STRU                stNasPlmnId;
    ...
    /* 保存当前驻留的PLMN信息 */
    pstLmmPlmnId = &(pstLmmSysInfo->stLteSysInfo.stSpecPlmnIdList.astSuitPlmnList[0]);
    NAS_MM_ConvertLmmPlmnToNasPLMN(pstLmmPlmnId->aucPlmnId, &stNasPlmnId);

    g_MmGlobalInfo.MsCsInfo.CurLai.PlmnId.ulMcc = stNasPlmnId.ulMcc;
    g_MmGlobalInfo.MsCsInfo.CurLai.PlmnId.ulMnc = stNasPlmnId.ulMnc;

    g_MmGlobalInfo.MsCsInfo.CurLai.ulLac  = (pstLmmSysInfo->stLteSysInfo.stTac.ucTac << 8) & 0xFF00;
    g_MmGlobalInfo.MsCsInfo.CurLai.ulLac |= pstLmmSysInfo->stLteSysInfo.stTac.ucTacCnt;
