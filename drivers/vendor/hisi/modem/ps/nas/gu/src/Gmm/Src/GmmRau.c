...
NAS_MSG_STRU *Gmm_RoutingAreaUpdateCompleteMsgMake(VOS_VOID)
{
    ...
    if (VOS_TRUE == bInterRatInfoFlg)
    {
        ...
        pRauComplete->aucNasMsg[usIeOffset++]   = GMM_IEI_INTER_RAT_HANDOVER_INFORMATION;
        pRauComplete->aucNasMsg[usIeOffset++]   = g_GmmInterRatInfoCtrl.ucDataLen;
        Gmm_MemCpy(&(pRauComplete->aucNasMsg[usIeOffset]),
...
)
{
    ...
    if (GMM_TRUE == Gmm_ComCmpLai(&pRaiTemp->Lai))
    {                                                                           /* LAI一致                                  */
        if (pRaiTemp->ucRac == g_GmmGlobalCtrl.SysInfo.Rai.ucRac)
        {                                                                       /* RAC一致                                  */
            ucRet = GMM_TRUE;                                                   /* 赋返回值                                 */
        }
    }
                                         ...
                                         )
{
    ...
    {                                                                           /* ucRauAttmptCnt小于5                      */
        ...
        if ((GMM_TRUE == Gmm_Com_CmpRai(&stGmmRai, g_GmmGlobalCtrl.ucNetMod))
