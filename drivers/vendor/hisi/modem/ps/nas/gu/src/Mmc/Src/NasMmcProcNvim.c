...
)
{
    switch(enNvimType)
    {
        case NAS_NVIM_CHANGE_REG_REJ_CAUSE_TYPE_INACTIVE:
            return NAS_MML_CHANGE_REG_REJ_CAUSE_TYPE_INACTIVE;

        case NAS_NVIM_CHANGE_REG_REJ_CAUSE_TYPE_CS_PS:
            return NAS_MML_CHANGE_REG_REJ_CAUSE_TYPE_CS_PS;

        case NAS_NVIM_CHANGE_REG_REJ_CAUSE_TYPE_CS_ONLY:
            return NAS_MML_CHANGE_REG_REJ_CAUSE_TYPE_CS_ONLY;

        case NAS_NVIM_CHANGE_REG_REJ_CAUSE_TYPE_PS_ONLY:
            return NAS_MML_CHANGE_REG_REJ_CAUSE_TYPE_PS_ONLY;

        default:
            return NAS_MML_CHANGE_REG_REJ_CAUSE_TYPE_BUTT;
    }
...
{
    NAS_NVIM_CHANGE_REG_REJECT_CAUSE_FLG_STRU stChangeRejCauFlg;
    VOS_UINT32                                ulLength;

    ulLength        = 0;

    PS_MEM_SET(&stChangeRejCauFlg, 0, sizeof(NAS_NVIM_CHANGE_REG_REJECT_CAUSE_FLG_STRU));

    stChangeRejCauFlg.enChangeRegRejCauCfg = NAS_MML_CHANGE_REG_REJ_CAUSE_TYPE_INACTIVE;

    NV_GetLength(en_NV_Item_ChangeRegRejectCause_Flg, &ulLength);
    if (ulLength > sizeof(NAS_NVIM_CHANGE_REG_REJECT_CAUSE_FLG_STRU))
    {
        return;
    }

    if (NV_OK != NV_Read(en_NV_Item_ChangeRegRejectCause_Flg,
                         &stChangeRejCauFlg, ulLength))
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC,
                     "NAS_MMC_ReadChangeRejCauFlgNvim():en_NV_Item_ChangeRejectCause_Flg Error");

        return;
    }

    if (NAS_NVIM_CHANGE_REG_REJ_CAUSE_TYPE_BUTT <= stChangeRejCauFlg.enChangeRegRejCauCfg)
    {
        NAS_ERROR_LOG(WUEPS_PID_MMC,
                      "NAS_MMC_ReadChangeRejCauFlgNvim():en_NV_Item_ChangeRejectCause_Flg Error");
    ...
    NAS_MML_SetChangeRegRejCauFlg(NAS_MMC_ConvertNvimChangeRegRejTypeToMmlType(stChangeRejCauFlg.enChangeRegRejCauCfg));

    NAS_MML_SetPreferredRegRejCause_HPLMN_EHPLMN(stChangeRejCauFlg.ucPreferredRegRejCau_HPLMN_EHPLMN);
    NAS_MML_SetPreferredRegRejCause_NOT_HPLMN_EHPLMN(stChangeRejCauFlg.ucPreferredRegRejCau_NOT_HPLMN_EHPLMN);
...
VOS_VOID NAS_MMC_ReadDisabledRatPlmnInfoNvim( VOS_VOID )
{
    ...
    if (stDisabledRatPlmnList.ulDisabledRatPlmnNum > NAS_MML_MAX_DISABLED_RAT_PLMN_NUM)
    {
        pstDisabledRatPlmnCfgInfo->ulDisabledRatPlmnNum = NAS_MML_MAX_DISABLED_RAT_PLMN_NUM;
    }

    if (stDisabledRatPlmnList.ulDisabledRatPlmnNum > 0)
    {
        PS_MEM_CPY(pstDisabledRatPlmnCfgInfo->astDisabledRatPlmnId,
                   stDisabledRatPlmnList.astDisabledRatPlmnId,
                   (sizeof(NAS_MML_PLMN_WITH_RAT_STRU) * NAS_MML_MAX_DISABLED_RAT_PLMN_NUM));
    }
...
VOS_VOID NAS_MMC_ReadUserCfgEhplmnInfoNvim( VOS_VOID )
{
    ...
    NAS_MML_SIM_FORMAT_PLMN_ID                              stSimPlmn;
    NAS_MML_PLMN_ID_STRU                                    stPlmn;
    NAS_MMC_NVIM_USER_CFG_EHPLMN_INFO_STRU                  stNvimCfgEhplmnInfo;
    NAS_MML_USER_CFG_EHPLMN_INFO_STRU                      *pstUserCfgEhplmnInfo;


    PS_MEM_SET(&stNvimCfgEhplmnInfo, 0x00, sizeof(stNvimCfgEhplmnInfo));

    pstUserCfgEhplmnInfo = NAS_MML_GetUserCfgEhplmnInfo();

    if ( NV_OK != NV_Read( en_NV_Item_User_Cfg_Ehplmn_Info,
                            &stNvimCfgEhplmnInfo, sizeof(NAS_MMC_NVIM_USER_CFG_EHPLMN_INFO_STRU)) )
    {
    ...
    /* 防止数组越界 */
    if ( stNvimCfgEhplmnInfo.ucImsiPlmnListNum > NAS_MMC_MAX_USER_CFG_IMSI_PLMN_NUM )
    {
        stNvimCfgEhplmnInfo.ucImsiPlmnListNum = NAS_MMC_MAX_USER_CFG_IMSI_PLMN_NUM;
    }

    /* 将USIM中保存的PLMN信息转换为内存中的信息 */
    for ( i = 0; i < stNvimCfgEhplmnInfo.ucImsiPlmnListNum; i++ )
    {
        PS_MEM_CPY(stSimPlmn.aucSimPlmn, stNvimCfgEhplmnInfo.astImsiPlmnList[i].aucSimPlmn, NAS_MML_SIM_PLMN_ID_LEN);
        NAS_MMC_ConvertSimPlmnToNasPLMN(&stSimPlmn, &stPlmn);

        if (VOS_TRUE == NAS_MML_IsPlmnIdValid(&stPlmn))
        {
            pstUserCfgEhplmnInfo->astImsiPlmnList[pstUserCfgEhplmnInfo->ucImsiPlmnListNum] = stPlmn;
            pstUserCfgEhplmnInfo->ucImsiPlmnListNum++;
        }
    }
    ...
    /* 防止数组越界 */
    if ( stNvimCfgEhplmnInfo.ucEhplmnListNum > NAS_MMC_MAX_USER_CFG_EHPLMN_NUM )
    {
        stNvimCfgEhplmnInfo.ucEhplmnListNum = NAS_MMC_MAX_USER_CFG_EHPLMN_NUM;
    }

    /* 将USIM中保存的PLMN信息转换为内存中的信息 */
    for ( i = 0; i < stNvimCfgEhplmnInfo.ucEhplmnListNum; i++ )
    {
        PS_MEM_CPY(stSimPlmn.aucSimPlmn, stNvimCfgEhplmnInfo.astEhPlmnList[i].aucSimPlmn, NAS_MML_SIM_PLMN_ID_LEN);
        NAS_MMC_ConvertSimPlmnToNasPLMN(&stSimPlmn, &stPlmn);

        if (VOS_TRUE == NAS_MML_IsPlmnIdValid(&stPlmn))
        {
            pstUserCfgEhplmnInfo->astEhPlmnList[pstUserCfgEhplmnInfo->ucEhplmnListNum] = stPlmn;
            pstUserCfgEhplmnInfo->ucEhplmnListNum++;
        }
    }
...
VOS_VOID NAS_MMC_ReadEnableLteTimerLenNvim(VOS_VOID)
{
    ...
    if (VOS_TRUE == stEnableLteTimerLen.ucNvimActiveFlag)
    {
        ...
        {
            NAS_MML_SetCsPsMode1EnableLteTimerLen(NAS_MML_ENABLE_LTE_TIMER_DEFAULT_LEN);
        }
        else
        {
            NAS_MML_SetCsPsMode1EnableLteTimerLen(stEnableLteTimerLen.ucCsPsMode1EnableLteTimerLen * NAS_MML_SIXTY_SECOND
                                         * NAS_MML_ONE_THOUSAND_MILLISECOND);
        }
