...
{ \
    TTF_MEM_SET_RCOV_ENT_TIMESTAMP(ulPoolId, VOS_GetSlice()); \
...
enum TTF_OM_EVENT_ID_ENUM
{
    ...
    TTF_EVENT_NODE_MEM_ABNML_INFO           = 0x0102,   /* _H2ASN_MsgChoice TTF_OM_EVENT_NODE_MEM_ABNML_STRU */
    TTF_EVENT_BUTT
};
...
/*****************************************************************************
 ...
 枚举说明  : 事件上报ID
*****************************************************************************/
enum TTF_MEM_LEAK_INFO_ID_ENUM
{
    ID_TRACE_TTF_MEM_LEAK_IND             = 0X0315,   /* _H2ASN_MsgChoice TTF_MEM_LEAK_INFO_IND_MSG */
    ID_TRACE_TTF_MEM_LEAK_BUTT
};
typedef VOS_UINT32 TTF_MEM_LEAK_INFO_ID_ENUM_UINT32;
typedef struct
{
    VOS_MSG_HEADER                                       /* 消息头 */        /* _H2ASN_Skip */
    TTF_MEM_LEAK_INFO_ID_ENUM_UINT32      ulMsgId;       /* 消息类型 */      /* _H2ASN_Skip */
...
enum TTF_NODE_MEM_TRIG_TYPE_ENUM
{
    TTF_NODE_MEM_CHK_ADDR_FAIL               = 0x0,
    TTF_NODE_MEM_CHK_MEM_FAIL,
    TTF_NODE_MEM_DUP_FREE,
    TTF_NODE_MEM_DUP_ALLOC,
    TTF_NODE_MEM_TRIG_TYPE_BUTT
};
typedef VOS_UINT8 TTF_NODE_MEM_TRIG_TYPE_ENUM_UINT8;
...
typedef struct
{
    ...
    VOS_UINT16  usFileId;
    VOS_UINT16  usLineNo;
    VOS_UINT16  usFreeFileId;
    VOS_UINT16  usFreeLineNo;
#endif
...
typedef struct
{
    ...
    VOS_VOID                               *pBaseAddr;
    volatile TTF_NODE_MEM_NODE_STRU        *pFreeNode[TTF_NODE_MEM_POOL_CNT];
}TTF_NODE_MEM_POOL_STRU;


typedef struct
{
    VOS_UINT8                ucTrigType;
    VOS_UINT8                ucRsv;
    VOS_UINT16               usPid;
    VOS_UINT16               usCurFileId;
    VOS_UINT16               usCurLineNum;
    TTF_NODE_MEM_NODE_STRU   stNodeMem;
    TTF_NODE_MEM_NODE_STRU  *pstNodeMemAddr;
}TTF_OM_EVENT_NODE_MEM_ABNML_STRU;
...
extern VOS_UINT32  TTF_MemBlkInit(VOS_VOID);
...
extern VOS_VOID   TTF_NodeMemFree_Debug(VOS_UINT32 ulPid, VOS_UINT8 * pMem, VOS_UINT16 usFileId, VOS_UINT16 usLineNo);
...
#define TTF_NODE_MEM_ALLOC(ulPid, size)     \
    TTF_NodeMemAlloc_Debug(ulPid, size, THIS_FILE_ID, __LINE__)

#define TTF_NODE_MEM_FREE(ulPid, pMem)     \
    TTF_NodeMemFree_Debug(ulPid, (VOS_UINT8 *)pMem, THIS_FILE_ID, __LINE__)

#define TTF_NODE_MEM_ALLOC_WITH_FILE_LINE(ulPid, size, ulFileID, ulLine)\
    TTF_NodeMemAlloc_Debug(ulPid, size, ulFileID, ulLine)
...
{ \
    ...
    { \
        ...
        { \
            ...
            pMem->pstDbgInfo->ulAllocTick       = OM_GetSlice(); \
            pMem->pstDbgInfo->usTraceFileId     = usFileID; \
            pMem->pstDbgInfo->usTraceLineNum    = usLineNum; \
        } \
        else \
        { \
            TTF_LOG(ulPid, PS_PRINT_ERROR, "Blk Mem debug info is null! \n"); \
        } \
    }\
}
...
#define TTF_MEM_DBG(pMem, usFileID, usLineNum)
#define TTF_MEM_RCOV_MEM(ulPoolId, ulPid, usFileId, usLine) \
     DRV_SYSTEM_ERROR(TTF_MEM_ALLOC_FAIL_SYSTEM_ERROR, usFileId, usLine, VOS_NULL_PTR, 0)
