...
extern int _snprintf(char *str, unsigned int size, const char *fmt, va_list args);
...
void bsp_trace(bsp_log_level_e log_level, bsp_module_e mod_id, char *fmt,...)
{
	...
	if(TRUE == om_get_hso_connect_flag())  
	{
        ...
        {
            //printk("bsp_trace: pBuffer is null!\n");

            return;
        }

		u32Pktlen = BSP_PRINT_BUF_LEN;

		/* 打包并拷贝数据 */
		
		if(pTempBuff.u32Size >= u32Pktlen)
		{
			memset(pTempBuff.pBuffer, 0, u32Pktlen);

            om_header_packet((u8*)pTempBuff.pBuffer, u32Pktlen, BSP_PRINT_STR_ID);

            print_str = (bsp_trace_txt_s *)(pTempBuff.pBuffer + sizeof(bsp_hisi_head_s)+sizeof(bsp_socp_head_s)+sizeof(bsp_om_head_s));

            print_str->mod_id = mod_id;
			print_str->level  = log_level;
			print_str->log_sn = g_om_global_info.print_sn++;

			p_fmt = (u8*)print_str->text;

            /* use om_sprintf */
            va_start(arglist, fmt);
			(void)_snprintf((char*)(p_fmt+1), (u32Pktlen-1), (char*)fmt, arglist);
            va_end(arglist);

            //trace_end = (char)(*(p_fmt + BSP_PRINT_DATA_LEN -1));
            trace_end = '\0';
            *(p_fmt + BSP_PRINT_DATA_LEN -1) = (u8)trace_end;

            g_om_global_info.trace_cnt = 0;

			/* 更新SOCP 通道的写指针*/
			bsp_socp_write_done(SOCP_ENCSRC_LPM3_BASE, u32Pktlen);
	...
	if(TRUE == om_get_hso_connect_flag())  
	{
		...
		if(pTempBuff.u32Size >= u32Pktlen)
		{
			...
			xprintf(fmt, arglist, (void*)om_console_xput, (void *)(p_fmt+1));
            va_end(arglist);
