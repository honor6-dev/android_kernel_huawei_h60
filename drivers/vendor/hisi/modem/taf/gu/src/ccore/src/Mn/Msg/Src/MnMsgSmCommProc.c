...
VOS_UINT32 NAS_MSG_SndOutsideContextData_Part1()
{
...
VOS_UINT32 NAS_MSG_SndOutsideContextData_Part2()
{
...
VOS_UINT32 NAS_MSG_SndOutsideContextData_Part3()
{
...
)
{
    ...
    {
        case EF_SMS_UST_ID:
            ulRet = MN_MSG_SndUsimGetFileReq(EF_SMS_UST_ID,0);
            break;

        case EF_SMS_SMSS_ID:
            ulRet = MN_MSG_SndUsimGetFileReq(EF_SMS_SMSS_ID,0);
            break;
        case EF_SMS_SMSP_ID:
#if ( VOS_WIN32 == VOS_OS_VER )
            if (0 == g_ucSetEfSmspNoRsp)
            {
                ulRet = MN_MSG_SndUsimGetFileReq(EF_SMS_SMSP_ID, MN_MSG_EF_FIRST_RECORD);
            }
#else
            ulRet = MN_MSG_SndUsimGetFileReq(EF_SMS_SMSP_ID, MN_MSG_EF_FIRST_RECORD);
#endif
            ...
            break;
        case EF_SMS_SMS_ID:
            ulRet = MN_MSG_SndUsimGetMaxRecReq(EF_SMS_SMS_ID);
            break;
        case EF_SMS_SMSR_ID:
            ulRet = MN_MSG_SndUsimGetFileReq(EF_SMS_SMSR_ID,0);
            break;
        default:
            break;
    }
...
)
{
    ...
    switch (usEfId)
    {
        case EF_SMS_SMSS_ID:
            ...
            {
                if (VOS_TRUE == MN_MSG_GetSmsFileRefreshFlag(EF_SMS_SMSS_ID))
                {
                    ulRet = MSG_GetUsimFileReq(EF_SMS_SMSS_ID);
        ...
        case EF_SMS_SMSP_ID:
            ...
            {
                if (VOS_TRUE == MN_MSG_GetSmsFileRefreshFlag(EF_SMS_SMSP_ID))
                {
                    ulRet = MSG_GetUsimFileReq(EF_SMS_SMSP_ID);
                    if (USIMM_API_SUCCESS == ulRet)
                    {
                         break;
                    }
        ...
        case EF_SMS_SMS_ID:
            ...
            {
                if (VOS_TRUE == MN_MSG_GetSmsFileRefreshFlag(EF_SMS_SMS_ID))
                {
                    ulRet = MSG_GetUsimFileReq(EF_SMS_SMS_ID);
                    if (USIMM_API_SUCCESS == ulRet)
                    {
                         break;
                    }
        ...
        case EF_SMS_SMSR_ID:
            ...
            {
                if (VOS_TRUE == MN_MSG_GetSmsFileRefreshFlag(EF_SMS_SMSR_ID))
                {
                    ulRet = MSG_GetUsimFileReq(EF_SMS_SMSR_ID);
                    if (USIMM_API_SUCCESS == ulRet)
                    {
                         break;
                    }
...
VOS_VOID MN_MSG_SmInit(VOS_VOID)
{
    ...
    if (VOS_NULL_PTR == gfpSmsFlash)
    {
        gfpSmsFlash = DRV_FILE_OPEN(g_pucSmsFileOnFlash,"rb+");
        if(VOS_NULL_PTR == gfpSmsFlash)
        {
...
VOS_UINT32 MN_MSG_CreateMoInfo(
    const MN_MSG_MO_ENTITY_STRU         *pstMoEntity
)
{
...
VOS_UINT32 MN_MSG_UpdateMoSaveInfo(
...
VOS_UINT32 MN_MSG_CreateMtInfo(VOS_VOID)
{
...
)
{
    ...
    if (MN_MSG_MEM_STORE_SIM == enMemStore)
    {
        ...
        {
            MN_WARN_LOG("MN_MSG_ReadSmsInfo: EF_SMS_SMS_ID Not Exist");
...
VOS_UINT32  MN_MSG_UpdateEachMemSmInfo(
    VOS_UINT32                          ulIndex,
...
)
{
    ...
    if (MN_MSG_MEM_STORE_SIM == enMemStore)
    {
        ...
        {
            MN_WARN_LOG("MN_MSG_ReadSmsrInfo: EF_SMS_SMSR_ID Not Exist");
            return MN_ERR_CLASS_SMS_FILE_NOEXIST;
        }
...
VOS_UINT32  MN_MSG_UpdateEachMemSmsrInfo(
    VOS_UINT8                           ucIndex,
    VOS_UINT8                           *pucEFContent,
...
VOS_UINT32 MN_MSG_UpdateEachMemSmspInfo(
    VOS_UINT8                           ucIndex,
    VOS_BOOL                            bUsed,
...
)
{
    ...
    if (MN_MSG_MEM_STORE_SIM == enMemStore)
    {
        ...
        {
            MN_WARN_LOG("MN_MSG_ReadSmspInfo: EF_SMS_SMSP_ID Not Exist");
            return MN_ERR_CLASS_SMS_FILE_NOEXIST;
        }
...
VOS_UINT32 MN_MSG_CheckUsimStatus(VOS_VOID)
{
    ...
    {
        MMA_GetUsimStatusFromUsim(&ucCardStatus, VOS_NULL_PTR);
...
)
{
    ...
    stENStru.EnvelopeType    = SI_STK_ENVELOPE_SMSCRTL;
    ...
    TAF_SDC_GetCurrentLai(aucLI, &stENStru.uEnvelope.MOSMSCtrl.LocInfo.ulLen);
...
)
{
    ...
    stDataDownLoad.EnvelopeType    = SI_STK_ENVELOPE_PPDOWN;
...
)
{
    ...
    if (VOS_TRUE == f_stMsgEfSmssInfo.bEfSmssState)
    {
        ulRet = MN_MSG_SndUsimSetFileReq(EF_SMS_SMSS_ID,aucWriteData,(VOS_UINT8)f_stMsgEfSmssInfo.ulFileLen,MN_MSG_USIM_DEF_REC);
        if (USIMM_API_SUCCESS != ulRet)
        {
        ...
        stUsimInfo.usEfId = EF_SMS_SMSS_ID;
...
)
{
    ...
    if (MN_MSG_MEM_STORE_SIM == enMemStore)
    {
        ...
        ulRet = MN_MSG_SndUsimSetFileReq(EF_SMS_SMS_ID,pucContentInfo,MN_MSG_EFSMS_LEN,pstWriteUsimInfo->stUsimInfo.ucRecIndex + 1);
        if (USIMM_API_SUCCESS != ulRet)
        {
...
)
{
    ...
    if (MN_MSG_MEM_STORE_SIM == enMemStore)
    {
        ...
        ulRet = MN_MSG_SndUsimSetFileReq(EF_SMS_SMS_ID,pucContentInfo,MN_MSG_EFSMS_LEN,pstSmaUsimInfo->ucRecIndex + 1);
        if (USIMM_API_SUCCESS != ulRet)
        {
            MN_WARN_LOG("MN_MSG_WriteSmsFile:Add Usim Node Failed");
            return MN_ERR_CLASS_SMS_UPDATE_USIM;
...
)
{
    ...
    if (MN_MSG_MEM_STORE_SIM == enMemStore)
    {
        ...
        ulRet = MN_MSG_SndUsimSetFileReq(EF_SMS_SMSR_ID,pucContentInfo,MN_MSG_EFSMSR_LEN,pstSmaUsimInfo->ucRecIndex + 1);
        if (USIMM_API_SUCCESS != ulRet)
        {
...
)
{
    ...
    if (MN_MSG_MEM_STORE_SIM == enMemStore)
    {
        if (VOS_FALSE == MN_MSG_IsUsimFileExist(EF_SMS_SMSP_ID))
        {
            MN_WARN_LOG("MN_MSG_WriteSmspFile: EF_SMS_SMSP_ID Not Exist");
            return MN_ERR_CLASS_SMS_FILE_NOEXIST;
        }

        ulRet = MN_MSG_SndUsimSetFileReq(EF_SMS_SMSP_ID,aucSmspContent,ucLen,pstSmaUsimInfo->ucRecIndex + 1);
        if (USIMM_API_SUCCESS != ulRet)
        {
...
)
{
    ...
    /*定位到索引位置*/
    lRst = DRV_FILE_LSEEK(fp, (VOS_INT32)(ulIndex * MN_MSG_EFSMS_LEN), SEEK_SET);
    if((-1) == lRst)
    {
    ...
    /*读文件*/
    /* 低软接口修改，调整了参数 */
    lRst = DRV_FILE_READ(pucSmContent, MN_MSG_EFSMS_LEN, 1, fp);
    if ((-1) == lRst)
    {
...
)
{
    ...
    {
        /*创建文件*/
        gfpSmsFlash = DRV_FILE_OPEN(g_pucSmsFileOnFlash,"wb+");
        if (VOS_NULL_PTR == gfpSmsFlash)
        {
    ...
    /*写文件*/
    /* 低软接口修改，调整了参数 */
    lRst = DRV_FILE_WRITE(pucSmContent, MN_MSG_EFSMS_LEN, 1, fp);
    if((-1) == lRst)
    {
...
)
{
    ...
    *penMeStorageEnable = stMeStorageParm.enMeStorageStatus;
    ...
    NV_GetLength(en_NV_Item_MO_SMS_CONTROL_FEATURE, &ulLength);
