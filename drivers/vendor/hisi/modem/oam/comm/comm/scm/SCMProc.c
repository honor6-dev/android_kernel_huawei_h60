...
VOS_VOID SCM_CoderDestReadCB(VOS_UINT32 ulDstChID)
{
    ...
    {
        DRV_SOCP_READ_DATA_DONE(ulDstChID, stBuffer.u32Size + stBuffer.u32RbSize);  /* 清空数据 */
        SCM_CODER_DST_ERR("SCM_CoderDestReadCB: Get RD error ", (VOS_UINT)ulDstChID,(VOS_UINT)0);/* 记录Log */
   ...
   if(VOS_NULL_PTR == g_astSCMCoderDstCfg[ulCfgNum].pfunc)
    ...
    {
        DRV_SOCP_READ_DATA_DONE(ulDstChID, stBuffer.u32Size + stBuffer.u32RbSize);  /* 清空数据 */
        SCM_CODER_DST_ERR("SCM_CoderDestReadCB:  stBuffer.pBuffer == VOS_NULL", (VOS_UINT)ulDstChID,(VOS_UINT)0);/* 记录Log */
        return;
    }
