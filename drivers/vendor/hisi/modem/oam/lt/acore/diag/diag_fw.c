...
VOS_UINT32 diag_FwForwardCmdToAgent(DIAG_FW_RX_INFO_STRU * pNode,VOS_UINT32 ulSendSize)
{
    ...
    if((DRV_RESET_CALLCBFUN_RESET_BEFORE == g_DiagResetingCcore)&&(ulCmdId == ulConCmdId))/*单独复位时,如果是Connect命令,返回错误*/
    {
        return ERR_MSP_UNAVAILABLE;
    }
    else if((DRV_RESET_CALLCBFUN_RESET_BEFORE == g_DiagResetingCcore)&&(ulCmdId == ulDisConCmdId))/*单独复位时,如果是DisConnect命令,只给A核*/
    {
        /*将命令转发给APP AGENT 处理*/
