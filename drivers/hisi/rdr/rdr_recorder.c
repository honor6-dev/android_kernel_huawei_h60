...
void rdr_reset2apr(char *path, u32 mod_id)
{
    ...
    if(((mod_id & HISI_RDR_MOD_CORE_MASK) == HISI_RDR_MOD_CORE_CP)
        ...
        snprintf(update_path,ARCH_NAME_MAX,"archive -i %s -i %s -i %s -i %s -i %s -o %s_modemcrash -z zip",
                  path,APR_CP_CSHELL_PATH_1,APR_CP_CSHELL_PATH_2,APR_CP_ETS_PATH_1,APR_CP_ETS_PATH_2,local_time);
