...
MAILBOX_LOCAL void mailbox_show_receive(struct mb_buff *mbuf)
{
	...
	if (MAILBOX_TRUE != mntn->sche.overflow) {
		mailbox_out(("Schedule Avg. slice:%4lu, total:%lu"RT,
					 (int)(mbuf->seq_num + 1) ? (mntn->sche.total)/(mbuf->seq_num + 1) : 0 , mbuf->seq_num));
	} else {
	...
	if (MAILBOX_TRUE != mntn->trans.overflow) {
		mailbox_out(("Transfers Avg. slice:%4d, total:%lu"RT,(int)((mbuf->seq_num + 1) ?
					 ((mntn->trans.total)/(mbuf->seq_num + 1)) : 0 ),  mbuf->seq_num));
	} else {
	...
	if (MAILBOX_TRUE != mntn->deal.overflow) {
		mailbox_out(("Call Back Avg. slice:%4lu, total:%lu"RT,(int)(mbuf->seq_num + 1) ?
					 (mntn->deal.total)/(mbuf->seq_num + 1) : 0, mbuf->seq_num ) );
	} else {
									   ...
									   int clear)
{
	...
	i = mntn->track_prob ;
	mailbox_out(("Latest transmit mails track:(%lu total)"RT, mbuf->seq_num));
	...
	mailbox_out(("Peak Traffic: %lu%%, Peak: 0x%x, Total: 0x%x"RT,
