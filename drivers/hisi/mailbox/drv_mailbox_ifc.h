...
#define IFC_ALIGN					sizeof(unsigned long)
...
struct ifc_param {
	unsigned long type;  /*记录参数类型，出参，入参等，IFC_INVAL*/
	unsigned long size;  /*记录定长参数大小*/
};

struct ifc_head {
	unsigned long		  scode;			/* 发起远程调用邮件编号。*/
	unsigned long		  rcode;			/* 执行方返回时的邮件号。*/
	unsigned long		  stamp;			/* 时间戳，用于筛选超时滞后响应。*/
	unsigned long		  retval;			/* 执行方函数调用返回值。*/
	unsigned long		  needret;			/* 执行方函数调用返回值。*/
	unsigned long		  argv[IFC_MAX_ARG];/* 执行方函数调用返回值,在out buff中的保存地址。*/
	unsigned long		  data; 			/* 消息头和填充空间之后的用户数据线性空间起始地址*/

#ifdef IFC_DEBUG_PARAM		/*对IFC中的宏中 参数进行调试*/
	struct ifc_param	  param_tbl[IFC_MAX_ARG];
	signed long 		  param_prob;
#endif
};
...
struct ifc_mb {
	...
	unsigned long		  total_size;			/* 邮件数据长度，包括:ifc消息头+空白填充+加数据的总长度*/
	unsigned long		  data_size;			/* ifc数据长度,不包括ifc消息头和填充部分*/
};
...
long name(_IFC_DECL##x(__VA_ARGS__) unsigned long timeout ) 																		\
{																																	\
	struct ifc_mb ifc_b;																											\
	struct ifc_arg *argo = ifc_b.argo;																								\
	unsigned long in_buf;																											\
	unsigned long *argv = ifc_b.head.argv;	/*此方法需要考虑编译器执行顺序差异*/													\
	unsigned long in_size = (_IFC_INSIZE##x(__VA_ARGS__));																			\
	argo=argo; argv=argv;																											\
...
long IFC_GEN_EXEC_NAME(name)(struct ifc_head * ifc_h)																				 \
{																																	 \
	struct ifc_mb ifc_b;																											 \
	unsigned long in_b, out_b;																										 \
	unsigned long *argv = ifc_h->argv;	/*需要考虑编译器执行顺序差异*/																 \
	unsigned long *argo;																											 \
	unsigned long out_size = (_IFC_OUTSIZE##x(&argv,__VA_ARGS__));																	 \
	...
	ifc_b.head.retval = (unsigned long)name(_IFC_PARAM##x(&in_b, &out_b, &argv, __VA_ARGS__));										 \
...
extern long mailbox_ifc_init_buf(
	struct ifc_mb *ifc_b,
...
extern long mailbox_ifc_send_no_wait(struct ifc_mb *ifc_b);

extern long mailbox_ifc_wait_resp(struct ifc_mb *ifc_b);

extern long mailbox_ifc_discard_buf(struct ifc_mb *ifc_b);

extern long mailbox_ifc_send_wait(struct ifc_mb *ifc_b, unsigned long timeout);

extern long mailbox_ifc_register_exec(
	unsigned long mailcode,
	long (*ife_cb)(struct ifc_head * ifc_h)
);

extern void __ifc_arg_out(struct ifc_arg ** ary, long db, long ab, long sb, long af);

extern void __ifc_push_arg(unsigned long * in, unsigned long ** av, unsigned long db, unsigned long ab, unsigned long sb, unsigned long af);

extern long __ifc_in_size(unsigned long db, unsigned long sb, unsigned long af);

extern void __ifc_dbg_arg(struct ifc_head *head, unsigned long db, unsigned long sb, unsigned long df);

extern long __ifc_out_size(unsigned long db, unsigned long sb, unsigned long **pp_af);

extern long __ifc_gen_arg(unsigned long *in, unsigned long *out, unsigned long db, unsigned long sb, unsigned long **pp_af);

extern void __ifc_copy_arg(unsigned long *in, unsigned long *out, unsigned long db, unsigned long sb, unsigned long **av , unsigned long **ao);

extern void __ifc_chk_arg(struct ifc_head *head, unsigned long db, unsigned long sb);
