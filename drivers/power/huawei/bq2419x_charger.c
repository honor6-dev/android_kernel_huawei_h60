...
#include <linux/switch_fsa9685.h>
...
bq2419x_charger_update_status(struct bq2419x_device_info *di)
{
    ...
    // power_supply_ovp misinformed when usb pull out
    /*
...
reg[9]:0x%x.\n", read_reg[9]);
    ...
    */
    ...
    if ((read_reg[9] & BQ2419x_BAT_FAULT_OVP) == BQ2419x_BAT_FAULT_OVP) {
        if (!udp_charge) {
            if (dsm_charger_error_found >= 0 && !regulation_voltage_changed) {
...
static void bq2419x_check_charger_type(struct bq2419x_device_info *di)
{
    ...
    if(type != CHARGER_TYPE_NONE)
    {
        ...
        case CHARGER_TYPE_DCP:
            ...
            hwlog_info("check charger type nonstandard to AC!!\n");
            break;
        case CHARGER_TYPE_CDP:
            di->event = CHARGER_TYPE_BC_USB;
            hwlog_info("check charger type nonstandard to BC USB!!\n");
            break;
        case CHARGER_TYPE_SDP:
            di->event = CHARGER_TYPE_USB;
            hwlog_info("check charger type nonstandard to USB!!\n");
            break;
        case CHARGER_TYPE_UNKNOWN:
            di->event = CHARGER_TYPE_NON_STANDARD;
            hwlog_info("check charger type nonstandard NO CHANGE!!\n");
            break;
        default:
            break;
        }
...
static void bq2419x_update_charge_current(struct bq2419x_device_info *di)
{
    switch(di->event){
    case CHARGER_TYPE_NON_STANDARD:
        di->enable_dpdm = DPDM_DIS;
        di->cin_limit = (di->cin_limit < IINLIM_1200) ? di->cin_limit : IINLIM_1200;
        di->currentmA = (di->currentmA < ICHG_1000) ? di->currentmA : ICHG_1000;
        di->charger_source = POWER_SUPPLY_TYPE_MAINS;
        break;
    case CHARGER_TYPE_USB:
        di->enable_dpdm = DPDM_DIS;
        di->cin_limit = (di->cin_limit < IINLIM_500) ? di->cin_limit : IINLIM_500;
        di->currentmA = (di->currentmA < ICHG_512) ? di->currentmA : ICHG_512;
        di->charger_source = POWER_SUPPLY_TYPE_USB;
        break;
    case CHARGER_TYPE_STANDARD:
    case CHARGER_TYPE_BC_USB:
        di->enable_dpdm = DPDM_DIS;
        di->cin_limit = (di->cin_limit < di->max_cin_currentmA) ? di->cin_limit : di->max_cin_currentmA;
        di->currentmA = (di->currentmA < di->max_currentmA) ? di->currentmA : di->max_currentmA;
        di->charger_source = POWER_SUPPLY_TYPE_MAINS;
        break;
    default:
        break;
...
static void bq2419x_charger_work(struct work_struct *work)
{
    ...
    bq2419x_check_charger_type(di);

    bq2419x_update_charge_current(di);
...
static ssize_t bq2419x_set_enable_timer(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val = 0;
    ...
    hwlog_info("set enable_timer = %ld\n", val);
...
static ssize_t bq2419x_show_enable_timer(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    unsigned long val;
...
static ssize_t bq2419x_show_shutdown_watchdog(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    unsigned long val;
...
static ssize_t bq2419x_set_shutdown_watchdog(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
    ...
    if ((strict_strtol(buf, 10, &val) < 0) || (val < 0) || (val > 1))
        return -EINVAL;
    prior_shutdown_wd = val;
    hwlog_info("set shutdown_wd = %d\n", prior_shutdown_wd);
    bq2419x_config_term_timer_reg(di);

    return status;
}

static ssize_t bq2419x_show_shutdown_q4(struct device *dev,
                  struct device_attribute *attr,
                  char *buf)
{
    unsigned long val;
...
static ssize_t bq2419x_set_shutdown_q4(struct device *dev,
                  struct device_attribute *attr,
                  const char *buf, size_t count)
{
    long val;
    ...
    if ((strict_strtol(buf, 10, &val) < 0) || (val < 0) || (val > 1))
        return -EINVAL;
    prior_shutdown_q4 = val;
    hwlog_info("set shutdown_q4 = %d\n", prior_shutdown_q4);
    bq2419x_config_misc_operation_reg(di);
    return status;
}
...
static DEVICE_ATTR(enable_timer, S_IWUSR | S_IRUGO,
                bq2419x_show_enable_timer,
                bq2419x_set_enable_timer);
static DEVICE_ATTR(shutdown_watchdog, S_IWUSR | S_IRUGO,
                bq2419x_show_shutdown_watchdog,
                bq2419x_set_shutdown_watchdog);
static DEVICE_ATTR(shutdown_q4, S_IWUSR | S_IRUGO,
                bq2419x_show_shutdown_q4,
                bq2419x_set_shutdown_q4);
