...
static int save_cali_param(void)
{
    ...
    {
        HISI_HI6421V300_COUL_INF("save cali param successn\n");
    }
    return ret;
}
static int do_save_offset_ops_set(const char *buffer,const struct kernel_param *kp)
{
...
{
    snprintf(buffer, PAGE_SIZE, "%d", do_save_offset_ret);
...
{
    int vol = hisi_hi6421v300_battery_voltage_uv();
...
void hisi_hi6421v300_print_cc_reg(int c)
{
    ...
    if (cnt)
        HISI_HI6421V300_COUL_INF("reg: cc_in = 0x%lx, cc_out = 0x%lx\n", g_cc_in, g_cc_out);
}
#endif
/**
...
static long calculate_cc_uah(void)
{
    ...
    unsigned long cl_in_time = 0;
    unsigned long cl_out_time = 0;
    long cc_uah_in = 0;
    long cc_uah_out = 0;
    long cc_uah = 0;
    s64 temp = 0;
...
static void clear_coul_time(void)
{
    ...
    if (di)
    {
        if (SR_DEVICE_WAKEUP == sr_cur_state)
        {
            ...
            {
                HISI_HI6421V300_COUL_ERR("[SR]%s(%d): di->sr_resume_time = %ld\n", __func__, __LINE__, di->sr_resume_time);
                di->sr_resume_time = 0;
            }
...
static void clear_coul_offset_reg(void)
{
    ...
    if (regval)
    {
        HISI_HI6421V300_COUL_ERR("Coul offset is not ZERO! reg:0x%lx\n",regval);
 ...
 */
static int interpolate_scalingfactor_fcc(struct hisi_hi6421v300_coul_device *di,
                                int cycles)
{
    /*
     * sf table could be null when no battery aging data is available, in
...
static int calculate_fcc_uah(struct hisi_hi6421v300_coul_device *di, int batt_temp,
                            int chargecycles)
{
...
static int calculate_pc(struct hisi_hi6421v300_coul_device *di, int ocv_uv, int batt_temp,
                            int chargecycles)
{
    int pc, scalefactor;
...
static int calculate_remaining_charge_uah(struct hisi_hi6421v300_coul_device *di,
...
static int calculate_termination_uuc(struct hisi_hi6421v300_coul_device *di,
...
static unsigned int nv_info_addr = 0;
...
static int __init early_parse_pmu_nv_addr_cmdline(char *p)
{
    ...
    nv_info_addr = (unsigned int)simple_strtoul(buf, &endptr, 16);
...
static int __init move_pmu_nv_info(void)
{
    ...
    HISI_HI6421V300_COUL_EVT("pmu_nv_addr=0x%x\n", (unsigned int)pmu_nv_addr);
...
static int get_initial_params(struct hisi_hi6421v300_coul_device *di)
{
    ...
    HISI_HI6421V300_COUL_INF("cycles=%d,reg_c=%d, reg_v=%d, start_cc=%ld\n"
...
int is_hi6421v300_battery_exist(void)
{
    ...
    if (HISI_HI6421V300_REG_READ(0x0) != 0x31)
    {
        HISI_HI6421V300_COUL_ERR("version reg is not 0x31\n");
        return 0;
    }
...
/**
 * is_hisi_hi6421v300_battery_full
 */
int is_hisi_hi6421v300_battery_full(void)
{
    struct hisi_hi6421v300_coul_device *di = g_hisi_hi6421v300_coul_dev;
    int ibat_ma = 0;
    int iterm_ma = 0;
    ...
    ibat_ma = -hisi_hi6421v300_battery_current();
    ...
    if (di->last_iavg_ma > MIN_CHARGING_CURRENT_OFFSET
        ...
        && ibat_ma < iterm_ma)
    {
        HISI_HI6421V300_COUL_INF("battery_full:ibat_ma=%d, when soc_real=%d, last_iavg_ma=%d\n",
...
int hisi_hi6421v300_battery_charge_param(struct battery_charge_param_s *param)
{
    struct hisi_hi6421v300_coul_device *di = g_hisi_hi6421v300_coul_dev;
...
static void get_initial_ocv(struct hisi_hi6421v300_coul_device *di)
{
    ...
    HISI_HI6421V300_COUL_INF("initial ocv = %d uv, start_cc = %ld uAh\n", ocv, di->cc_start_value);
}
...
static int calculate_state_of_charge(struct hisi_hi6421v300_coul_device *di)
{
    ...
    if (di->batt_limit_fcc){
        ...
        HISI_HI6421V300_COUL_INF("use limit_FCC! %duAh\n", fcc_uah);
    }
 ...
 static void calculate_soc_work(struct work_struct *work)
{
   ...
   else if (cali_cnt % (CALIBRATE_INTERVAL / di->soc_work_interval) == 1)
   {
    ...
    if (di->charging_state == CHARGING_STATE_CHARGE_DONE){
        ...
        HISI_HI6421V300_COUL_INF("sleep_cc=%ld, sleep_time=%ld\n", sleep_cc, sleep_time);
        ...
        HISI_HI6421V300_COUL_INF("sleep_current=%ld\n", sleep_current);
...
void ss_cc_show(void)
{
    ...
    printk(KERN_ERR "ccout_reg=0x%x,ccin_reg = 0x%x,cc = %ld uah \n"
           "cl_out_time=%ld, cl_in_time=%ld, cc_adjust=%ld uah\n",
...
void ss_di_show(void)
{
    ...
    printk(KERN_ERR"get_cc_end_time = %ld\n", di->get_cc_end_time);
    printk(KERN_ERR"suspend_time = %ld\n", di->suspend_time);
    printk(KERN_ERR"charging_begin_cc = %ld\n", di->charging_begin_cc);
    printk(KERN_ERR"suspend_cc = %ld\n", di->suspend_cc);
    printk(KERN_ERR"last_time = %ld\n", di->last_time);
    printk(KERN_ERR"last_cc = %ld\n", di->last_cc);
...
static ssize_t hisi_hi6421v300_show_gaugelog(struct device_driver *driver, char *buf)
{
    ...
    snprintf(buf, PAGE_SIZE, "%-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-6d  %-8d  %-6d  %-3d  %-5d  %-6d  %-6d  %-5d  %-6d  %-4d  %-7d  %-5d  %-5d  ",
                glog->uuc_pc, glog->soc_est, glog->rbat_calc, glog->ocv_for_delrc, glog->vavg, glog->cavg, glog->delta_ocv, glog->delta_rc_mid, glog->delta_rc_100, glog->iavg_ua, glog->soc_before_limit,
                voltage,  (signed short)cur, ufcapacity, capacity, afcapacity, rm, fcc, uuc, cc, delta_rc, temp, ocv, rbatt, di->batt_limit_fcc/1000);

    return strlen(buf);
}

static ssize_t hisi_hi6421v300_set_hand_chg_capacity_flag(struct device_driver *driver, const char *buf, size_t count)
{
    long val = 0;
    int status = count;
...
static ssize_t hisi_hi6421v300_show_hand_chg_capacity_flag(struct device_driver *driver, char *buf)
{
    unsigned long val;
...
static ssize_t hisi_hi6421v300_set_input_capacity(struct device_driver *driver,const char *buf, size_t count)
{
    long val = 0;
    int status = count;
...
static ssize_t hisi_hi6421v300_show_input_capacity(struct device_driver *driver, char *buf)
{
    unsigned long val;
...
static DRIVER_ATTR(gaugelog, S_IWUSR | S_IWGRP | S_IRUGO, hisi_hi6421v300_show_gaugelog,NULL);
...
static int hisi_hi6421v300_coul_probe(struct platform_device *pdev)
{
    ...
    {
        short ocvreg = 0x0;
        HISI_HI6421V300_COUL_ERR("hisi_hi6421v300 debug reg is not 0x0\n");
    ...
    clear_coul_offset_reg();
    retval = driver_create_file(&(hisi_hi6421v300_coul_driver.driver), &driver_attr_gaugelog);
    if (0 != retval)
    {
        HISI_HI6421V300_COUL_ERR("failed to create sysfs entry(gaugelog): %d\n", retval);
        return -1;
    }
    retval = driver_create_file(&(hisi_hi6421v300_coul_driver.driver), &driver_attr_hand_chg_capacity_flag);
    if (0 != retval)
    {
        HISI_HI6421V300_COUL_ERR("failed to create sysfs entry(hand_chg_capacity_flag): %d\n", retval);
        return -1;
    }
    retval = driver_create_file(&(hisi_hi6421v300_coul_driver.driver), &driver_attr_input_capacity);
    if (0 != retval)
    {
        HISI_HI6421V300_COUL_ERR("failed to create sysfs entry(input_capacity): %d\n", retval);
        return -1;
    }
    retval = driver_create_file(&(hisi_hi6421v300_coul_driver.driver), &driver_attr_abs_cc);
    if (0 != retval) {
        HISI_HI6421V300_COUL_ERR("failed to create sysfs entry(abs_cc): %d\n", retval);
        return -1;
    }
    ...
    /*set battery data*/
    di->batt_id_vol = hisi_hi6421v300_battery_id_voltage();
    di->batt_data = get_battery_data(di->batt_id_vol);
    HISI_HI6421V300_COUL_EVT("battery type is %s\n", di->batt_data->batt_brand);
...
coul_no_battery:
    ...
    coul_ops->is_battery_full = is_hisi_hi6421v300_battery_full;
    ...
    coul_ops->battery_charge_param = hisi_hi6421v300_battery_charge_param;
...
static void hisi_hi6421v300_coul_shutdown(struct platform_device *pdev)
{
    ...
    disable_irq(di->irq);
    cancel_delayed_work_sync(&di->notifier_work);
    cancel_delayed_work_sync(&di->read_temperature_delayed_work);
    ...
    pm_message_t state)
{
    ...
    {
        clear_sr_time_array();
        HISI_HI6421V300_COUL_INF("[SR]%s(%d): wakeup_time(%ld) > SR_MAX_RESUME_TIME(%d)\n", __func__, __LINE__, wakeup_time, SR_MAX_RESUME_TIME);
    }
    ...
    {
        HISI_HI6421V300_COUL_ERR("[SR]%s(%d): wakeup_time=%ld, di->sr_suspend_time=%ld, di->sr_resume_time=%ld\n",
            __func__, __LINE__, wakeup_time, di->sr_suspend_time, di->sr_resume_time);
    }

    sr_cur_state = SR_DEVICE_SLEEP;
    DI_UNLOCK();
    HISI_HI6421V300_COUL_INF("SUSPEND! cc=%ld, time=%ld\n", di->suspend_cc,
                       di->suspend_time);
    cancel_delayed_work(&di->read_temperature_delayed_work);
    if (di->batt_exist)
    {
...
static long sr_get_duty_ratio(void) {
    ...
    if (total_sleep_time + total_wakeup_time >= SR_TOTAL_TIME)
    {
        ...
        HISI_HI6421V300_COUL_INF("[SR]%s(%d): total_wakeup=%lds, total_sleep=%lds, duty_ratio=%ld\n",
...
static int sr_need_update_ocv(struct hisi_hi6421v300_coul_device *di)
{
    ...
    HISI_HI6421V300_COUL_INF("[SR]%s(%d): need_update, last_sleep=%lds, last_wakeup=%lds, duty_ratio=%ld\n",
...
static int ocv_could_update(struct hisi_hi6421v300_coul_device *di)
{
    ...
    {
        HISI_HI6421V300_COUL_INF("[SR]Can't update ocv, sleep_current=%ld ma, sleep_time=%ld s\n", sleep_current, sleep_time);
        return 0;
    }
    ...
    {
        HISI_HI6421V300_COUL_ERR("[SR]%s(%d): sleep_time = %ld\n",  __func__, __LINE__, sleep_time);
        return 0;
    }

    HISI_HI6421V300_COUL_INF("[SR]going to update ocv, sleep_time=%lds, sleep_current=%ld ma\n", sleep_time, sleep_current);
    return 1;
}
...
static int hisi_hi6421v300_coul_resume(struct platform_device *pdev)
{
    ...
    {
        HISI_HI6421V300_COUL_ERR("[SR]%s(%d): sr_sleep_time = %ld\n", __func__, __LINE__, sr_sleep_time);
    }
