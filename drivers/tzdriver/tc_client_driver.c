...
atomic_t volatile atomic_pre_allocted = ATOMIC_INIT(0);
...
static unsigned int TC_NS_incomplete_proceed(TC_NS_SMC_CMD *smc_cmd, unsigned int agent_id)
{
    ...
    do{
        ret2 = wait_event_freezable(event_data->send_response_wq, event_data->send_flag);
        if(ret2){
            TCERR("TC_NS_incomplete_proceed:wait_event_freezable ret is %d\n", ret2);
        }
    }while(!event_data->send_flag);
    ...
    if(operation){
        for(i = 0; i < 4; i++){
            ...
            if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
                ...
                if(virt_addr_valid(client_param->memref.size_addr)){
                    buffer_size = *(unsigned int *)client_param->memref.size_addr;
                }else if(copy_from_user((void *)&buffer_size, (void *)client_param->memref.size_addr,
                    sizeof(unsigned int))) {
                ...
                temp_buf = (void *)kzalloc(buffer_size, GFP_KERNEL);
                if(!temp_buf){
                ...
                if((param_type == TEEC_MEMREF_TEMP_INPUT) ||
                    ...
                    if (virt_addr_valid(client_param->memref.buffer))   //buffer is in kernel space(kernel driver call kernel TEE API)
                    {
                        memcpy(temp_buf, (void *)(client_param->memref.buffer),buffer_size);
                    }
                    else   //buffer is in user space(CA call  TEE API)
                    {
                        if(copy_from_user(temp_buf, (void *)client_param->memref.buffer, buffer_size)) {
            ...
            //allocate buffer
                ...
                if(virt_addr_valid(client_param->memref.size_addr)){
                    buffer_size = *(unsigned int *)client_param->memref.size_addr;
                }else if(copy_from_user(&buffer_size, (void *)client_param->memref.size_addr,
                    sizeof(unsigned int))) {
            ...
            //value
                ...
                if(virt_addr_valid(client_param->value.a_addr)){
                    operation->params[i].value.a = *(unsigned int *)client_param->value.a_addr;
                }else if(copy_from_user(&operation->params[i].value.a, (void *)client_param->value.a_addr,
                    sizeof(unsigned int))) {
                ...
                if(virt_addr_valid(client_param->value.b_addr)){
                    operation->params[i].value.b = *(unsigned int *)client_param->value.b_addr;
                }else if(copy_from_user(&operation->params[i].value.b, (void *)client_param->value.b_addr,
                    sizeof(unsigned int))) {
            ...
            if((param_type ==  TEEC_MEMREF_TEMP_OUTPUT)||
                ...
                else   //buffer is in user space(CA call  TEE API)
                {
                    if(copy_to_user((void *)client_param->memref.buffer,
                        local_temp_buffer[i], (unsigned long)operation->params[i].memref.size)){
                        TCERR("copy to user failed while copying tempbuf\n");
                        ret = -ENOMEM;
                        goto error;
                    }
                    /* update size */
                    if(copy_to_user((void *)client_param->memref.size_addr,
                        &operation->params[i].memref.size, sizeof(unsigned int))){
                ...
                if(virt_addr_valid(client_param->memref.size_addr)){
                    *(unsigned int *)client_param->memref.size_addr = operation->params[i].memref.size;
                }else if(copy_to_user((void *)client_param->memref.size_addr,
                    &operation->params[i].memref.size, sizeof(unsigned int))){
                    TCERR("copy to user failed while copying allocate memory size\n");
                    ret = -ENOMEM;
                    goto error;
                }
                ...
                //value
                if(virt_addr_valid(client_param->value.a_addr)){
                    *(unsigned int *)client_param->value.a_addr = operation->params[i].value.a;
                }else if(copy_to_user((void *)client_param->value.a_addr,
                    &operation->params[i].value.a, sizeof(unsigned int))){
                    TCERR("copy to user failed while copying client_param->value.a_addr\n");
                    ret = -ENOMEM;
                    goto error;
                }

                if(virt_addr_valid(client_param->value.b_addr)){
                    *(unsigned int *)client_param->value.b_addr = operation->params[i].value.b;
                }else if(copy_to_user((void *)client_param->value.b_addr,
                    &operation->params[i].value.b, sizeof(unsigned int))){
                    TCERR("copy to user failed while copying client_param->value.b_addr\n");
                    ret = -ENOMEM;
                    goto error;
                }
            }
        ...
        if(operation){
            for(i = 0; i < 4; i++){
                ...
                if((param_type ==  TEEC_MEMREF_TEMP_OUTPUT)||
                    ...
                    if (buffer_size >= operation->params[i].memref.size)
                    {
...
static int TC_NS_get_app_cert(TC_NS_ClientContext *client_context)
{
    int ret = -EINVAL;
    struct __smc_event_data *event_data;
    unsigned long flags;
    int find_flag=0;
    int ret2;
    struct sec_storage_kernel_t *sec_storage_kernel_control;
    char * cert_buffer;
    int package_name_len,public_key_len;
    //char pkg_name[100]={0};
    int ca_uid, uid_len;

    spin_lock_irqsave(&agent_control.lock, flags);
    list_for_each_entry(event_data, &agent_control.agent_list, head){
        if(event_data->agent_id == AGENT_FS_ID){
            find_flag = 1;
            break;
        }
    }
    ...
    if (-1 == ca_uid) {
        TCERR("get ca uid failed\n");
        return TEEC_ERROR_GENERIC;
    }

    mutex_lock(&event_data->work_lock);

    sec_storage_kernel_control = (struct sec_storage_kernel_t *)(event_data->buffer);
    sec_storage_kernel_control->cmd = nor_getprocinfo;
    sec_storage_kernel_control->args.get_proc_info.pid = current->pid;
    sec_storage_kernel_control->args.get_proc_info.uid = ca_uid;
    ...
    do{
        ret2 = wait_event_freezable(event_data->send_response_wq, event_data->send_flag);
        if(ret2){
            TCERR("wait_event_freezable ret is %d\n", ret2);
        }
    }while(!event_data->send_flag);
    event_data->send_flag = 0;
    ...
    cert_buffer = (char *)(sec_storage_kernel_control->args.get_proc_info.proc_inf);
    memcpy(&package_name_len, cert_buffer, sizeof(package_name_len));
    TCDEBUG("package_name_len is %d\n", package_name_len);
    if (copy_to_user((void *)(client_context->params[3].memref.size_addr), &package_name_len, sizeof(package_name_len))) {
        TCERR("package name len copy failed\n");
        ret = -1;
        goto error;
    }
    cert_buffer+=sizeof(package_name_len);
    //memcpy(pkg_name, cert_buffer, package_name_len);
    //TCDEBUG("package name is %s\n", pkg_name);
    if (copy_to_user((void *)(client_context->params[3].memref.buffer), cert_buffer, package_name_len)) {
        TCERR("package name copy failed\n");
        ret = -1;
        goto error;
    }
    cert_buffer+=package_name_len;

    /*get public key len and public key*/
    memcpy(&public_key_len, cert_buffer, sizeof(public_key_len));
    TCDEBUG("publick_key_len is %d\n", public_key_len);
    if (0 != public_key_len) {
        if (copy_to_user((void *)(client_context->params[2].memref.size_addr), &public_key_len, sizeof(public_key_len))) {
            TCERR("public key len copy failed\n");
            ret = -1;
            goto error;
        }
        cert_buffer+=sizeof(public_key_len);
        TCDEBUG("public_key first 4 bytes %x,%x,%x,%x\n", cert_buffer[0], cert_buffer[1], cert_buffer[2], cert_buffer[3]);
        if (copy_to_user((void *)(client_context->params[2].memref.buffer), cert_buffer, public_key_len)) {
            TCERR("public key copy failed\n");
            ret = -1;
            goto error;
        }
        cert_buffer+=public_key_len;
    }
    else {
        ...
        uid_len = sizeof(ca_uid);
        if (copy_to_user((void *)(client_context->params[2].memref.size_addr), &uid_len, sizeof(uid_len))) {
            TCERR("public key len copy failed\n");
            ret = -1;
            goto error;
        }
        if (copy_to_user((void *)(client_context->params[2].memref.buffer), &ca_uid, uid_len)) {
            TCERR("public key copy failed\n");
            ret = -1;
            goto error;
        }
...
find_service:
    ...
    if (client_context.login.method == TEEC_LOGIN_IDENTIFY) {
        TCDEBUG("login method is IDENTIFY\n");
        ret = TC_NS_get_app_cert(&client_context);
        if (ret != 0) {
            TCERR("TC_NS_get_app_cert failed ret =%d\n", ret);
            goto error;
        }
    }
                    ...
                    list_for_each_entry_safe(session, session_temp, &service->session_list, head){
                        if(session->session_id == client_context.session_id){
                            ...
                            if (ret == TEEC_SUCCESS) {
                                ...
                                TCERR("close session smc failed, ret=%d, client_context.returns.code=%x, client_context.returns.origin=%x\n", ret, client_context.returns.code, client_context.returns.origin);
                            }
                            list_del(&session->head);
                            kfree(session);
                            ret = TEEC_SUCCESS;
        ...
        if(dev_file->dev_file_id == dev_id){
            ...
            list_for_each_entry_safe(shared_mem, shared_mem_temp, &dev_file->shared_mem_list, head){
                if(shared_mem && shared_mem->user_addr == (void *)user_addr){
                    ...
                    if(shared_mem->kernel_addr && (shared_mem->kernel_addr != g_mem_pre_allocated)){
                    ...
                    if(shared_mem->kernel_addr == g_mem_pre_allocated){
                        atomic_set(&atomic_pre_allocted, 0);
                    }
                    if(shared_mem)
                        kfree(shared_mem);
...
static int TC_NS_wait_event(unsigned long agent_id)
{
    ...
    if(ret){
        TCERR("TC_NS_wait_event:wait_event_freezable ret is %d\n", ret);
    }
...
static int TC_NS_ClientMalloc(struct file *filp, struct vm_area_struct *vma){
    ...
    if(!addr) {
        ...
        if(!g_mem_pre_allocated || (len > PRE_ALLOCATE_SIZE) || (1 == atomic_read(&atomic_pre_allocted))){
            TCERR("error:g_mem_pre_allocated is null or size is too large\n");
            ret = -ENOMEM;
            return ret;
        }else{
            atomic_set(&atomic_pre_allocted, 1);
...
*/
    ...
    g_mem_pre_allocated = (void*) __get_free_pages(GFP_KERNEL, get_order(ROUND_UP(PRE_ALLOCATE_SIZE, SZ_4K)));
...
static void TC_NS_ClientExit(void)
{
    ...
    kfree(p_smc_data);
    if(g_mem_pre_allocated)
        free_pages((unsigned long)g_mem_pre_allocated,get_order(ROUND_UP(PRE_ALLOCATE_SIZE, SZ_4K)));
    atomic_set(&atomic_pre_allocted, 0);
}
