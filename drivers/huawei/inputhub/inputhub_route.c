...
static bool during_recovery_iom3(void)//false - send direct  true - update
{
	bool ret = false;
...
static int inputhub_mcu_write_cmd(const void *buf, unsigned long length)
{
    ...
    is_in_recovery = during_recovery_iom3();//stop all cmds when in recovery
...
int send_motion_cmd_internal(int tag, obj_cmd_t cmd, motion_type_t type, bool use_lock)
{
    ...
    if (CMD_CMN_OPEN_REQ == cmd) {
        if (!sensor_info_isensor_version) {
            ...
            hwlog_info("send_motion_cmd  cmd:%d  motion: %d !", cmd, pkt_motion.motion_type);
            if (use_lock) {
