...
#define GPIO_SENSORHUB_RESET 74
...
static uint8_t check_file_list(uint8_t file_count, uint16_t *file_list)
{
	...
	/*5 means DMP driver, 6 means DMP image*/
	if (map[DMP_DRIVER_FILE_ID] >= 1) {
		map[DMP_IMG_FILE_ID] = 1;
	}
...
static int mcu_i2c_rw(uint8_t bus_num, uint8_t i2c_add, uint8_t register_add, uint8_t rw, int len, uint8_t *buf)
{
    ...
    read_info_t	pkg_mcu;
    pkt_i2c_read_req_t pkt_i2c_read;
    pkt_i2c_write_req_t pkt_i2c_write;

    memset(&pkt_i2c_read, 0, sizeof(pkt_i2c_read));
    memset(&pkg_mcu, 0, sizeof(pkt_i2c_write));
    memset(&pkg_ap, 0, sizeof(pkg_ap));
    memset(&pkg_mcu, 0, sizeof(pkg_mcu));
...
static int init_sensors_cfg_data_from_dts(void)
{
	...
	bool is_invensense_exit = false;

       sensor_info_isensor_version = check_sensorhub_isensor_version();

	is_invensense_exit = check_invensense_exist();
	if (true == is_invensense_exit) {
		gpio_request(GPIO_SENSORHUB_RESET,"sensorhub");
		gpio_direction_output(GPIO_SENSORHUB_RESET,1);
		msleep(100);
	}

	hwlog_info("is_invensense_exit=%d\n",is_invensense_exit);
	...
	for_each_node_with_property(dn, "sensor_type")
	{
		...
		}else if(!strncmp(sensor_ty, "mag", sizeof("mag")))
		{
			...
			if (true == is_invensense_exit && 0 == mag_flag) {//set INVENSENSE 20628 bypass mode to detect mag
			...
			if (true == is_invensense_exit && 0 == mag_flag) {//finished getting mag dev from dts, then recovery, not bypass
