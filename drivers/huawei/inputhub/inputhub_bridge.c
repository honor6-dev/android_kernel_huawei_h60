...
static atomic_t iom3_rec_state;
...
retry:
    ...
    if ((-ETIMEOUT) == tx_task->tx_error) {
        ...
        if(--try_times > 0){
                hwlog_err("send failed retry again %d\n", try_times);
                goto retry;
        }
        ret = -1;
    }else{
...
static void reset_i2c_0_controller(void)
{
	...
	unsigned int volatile *rstst_addr = (unsigned int volatile *)ioremap(PERRSTSTAT2_ADDR, 4);

	if ((!rsten_addr) ||(!rstdis_addr) ||(!rstst_addr)) {
		hwlog_err("ioremap i2c reset reg addr failed, 0x%x, 0x%x, 0x%x\n",
			(unsigned int)rsten_addr, (unsigned int)rstdis_addr, (unsigned int)rstst_addr);
		return;
	}

	local_irq_save(flags);
	reg32_val = readl(rstst_addr);
	reg32_val |= I2C_0_RST_MASK;
	writel(reg32_val, rsten_addr);
	udelay(5);
	writel(reg32_val, rstdis_addr);
