...
static int fwu_wait_for_idle(int timeout_ms)
{
	...
	do {
		...
		TS_LOG_DEBUG("flash command 0x%x, status 0x%x\n", fwu->command, fwu->flash_status);
...
static int fwu_write_bootloader_id(void)
{
	...
	TS_LOG_INFO("bootloader id addr = 0x%x\n", fwu->f34_fd.data_base_addr + fwu->blk_data_off);
	retval = fwu->fn_ptr->write(fwu->rmi4_data,
			fwu->f34_fd.data_base_addr + fwu->blk_data_off,
			fwu->bootloader_id,
			sizeof(fwu->bootloader_id));
...
static int fwu_enter_flash_prog(void)
{
	...
	TS_LOG_DEBUG("fwu->command = 0x%x, fwu->flash_status = 0x%x\n", fwu->command, fwu->flash_status);
...
int synaptics_get_fw_data_boot(char *file_name)
{
	...
	snprintf(firmware_name, sizeof(firmware_name), "ts/%s.img", file_name);
	firmware_name[RMI_PRODUCT_ID_LENGTH + file_name_size] = 0;

	TS_LOG_INFO("file_name size = %d, Requesting firmware image %s\n",
