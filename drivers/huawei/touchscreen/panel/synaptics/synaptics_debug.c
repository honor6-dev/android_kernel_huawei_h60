...
static int f54_delta_tx_report (void)
{
	...
	for( tx_n = 0; tx_n < f54->rmi4_data->num_of_tx - 1; tx_n++)
	{
		for(rx_n = 0; rx_n < f54->rmi4_data->num_of_rx; rx_n++)
		{
			...
			if(tx_n+1 != f54->rmi4_data->num_of_tx)
				printk("%5d",Rawimage_delta_tx);
...
static int synaptics_read_f34(void)
{
	...
	if (V5 == f54->bl_version) {
		...
		retval = f54->fn_ptr->read(f54->rmi4_data, f54->rmi4_data->f01_ctrl_base_addr+RX_NUMBER, &f54->rmi4_data->num_of_rx, 1);
		if (retval < 0){
			TS_LOG_ERR("Could not read RX value from 0x%04x\n", f54->rmi4_data->f01_ctrl_base_addr + RX_NUMBER);
			return -EINVAL;
		}

		retval = f54->fn_ptr->read(f54->rmi4_data, f54->rmi4_data->f01_ctrl_base_addr + TX_NUMBER, &f54->rmi4_data->num_of_tx, 1);
		if (retval < 0){
