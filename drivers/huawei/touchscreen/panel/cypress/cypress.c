...
#ifndef CONFIG_OF
	rc = get_hw_config_int(CY_IRQ_GPIO, &chip_data->irq_gpio, NULL);
	if (!rc) {
		TS_LOG_ERR("read irq gpio failed\n");
		goto err;
	}

	rc = get_hw_config_int(CY_RST_GPIO, &chip_data->reset_gpio, NULL);
	if (!rc) {
		TS_LOG_ERR("read reset gpio failed\n");
		goto err;
	}

	rc = get_hw_config_int(CY_IRQ_CFG, &chip_data->irq_config, NULL);
	if (!rc) {
		TS_LOG_ERR("read failed\n");
		goto err;
	}
