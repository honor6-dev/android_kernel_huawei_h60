...
static ssize_t ts_gesture_wake_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	u32 gesture_wakeup_command = 0;
	int error;
	struct ts_cmd_node *cmd = NULL;
	struct ts_palm_info *info = NULL;
	...
	error = sscanf(buf, "%u",&gesture_wakeup_command);
	if (error <= 0) {
		TS_LOG_ERR("sscanf gesture wakeup return invaild :%d\n", error);
		error = -EINVAL;
		goto out;
	}

	TS_LOG_INFO("%s: value=%u\n",__func__,gesture_wakeup_command);

	g_ts_data.chip_data->rs_info.ts_gesture_command_data = gesture_wakeup_command;
	if ((gesture_wakeup_command & TS_GESTURE_ENABLE(GESTURE_DOUBLE_CLICK)) ||
		(gesture_wakeup_command & TS_GESTURE_ENABLE(GESTURE_SLIDE_ENABLE))||
			(gesture_wakeup_command &TS_GESTURE_ENABLE(GESTURE_LETTER_ENABLE))) {
				g_ts_data.chip_data->rs_info.sleep_mode = TS_GESTURE_MODE;
				gesture_func = true;
	} else {
		g_ts_data.chip_data->rs_info.sleep_mode = TS_POWER_OFF_MODE;
		gesture_func = false;
		TS_LOG_INFO("no gesture wakeup mode\n");
	}
	if (gesture_wakeup_command & TS_GESTURE_ENABLE(GESTURE_PALM_COVERED)) {
		g_ts_data.chip_data->rs_info.gesture_palm_set_flag = true;
	} else {
		g_ts_data.chip_data->rs_info.gesture_palm_set_flag = false;
	}
	...
	info = (struct ts_palm_info *)kzalloc(sizeof(struct ts_palm_info), GFP_KERNEL);
	if (!info) {
		TS_LOG_ERR("malloc failed\n");
		error = -ENOMEM;
		goto out;
	...
	info->op_action = TS_ACTION_WRITE;
	info->palm_switch = g_ts_data.chip_data->rs_info.gesture_palm_set_flag;
...
static ssize_t ts_gesture_wake_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int error = NO_ERR;

	TS_LOG_INFO("ts_gesture_wake_show called\n");
	...
	error = snprintf(buf, MAX_STR_LEN, "%d\n", g_ts_data.chip_data->rs_info.ts_gesture_command_data);

out:
	TS_LOG_INFO("ts_gesture_wake_show done\n");
	return error;
}

static ssize_t ts_gesture_wake_position_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int error = NO_ERR;
	int i = 0;
	char temp[9] = {0};
	u32 easywake_position[MAX_POSITON_NUMS] = {0};

	TS_LOG_INFO("ts_gesture_wake_position_show called\n");
	...
	for(i = 0; i < MAX_POSITON_NUMS; i++) {
		easywake_position[i] = (u32)g_ts_data.chip_data->rs_info.easywake_position[i];
		error = snprintf(temp, (sizeof(u32)*2+1), "%08x",easywake_position[i]);
	...
	error = (strlen(buf)+1);
out:
	TS_LOG_INFO("ts_gesture_wake_position_show done\n");
	return error;
}
...
static DEVICE_ATTR(touch_gesture_wakeup, S_IRUSR|S_IWUSR, ts_gesture_wake_show, ts_gesture_wake_store);
static DEVICE_ATTR(dsm_debug, S_IRUSR | S_IRGRP | S_IROTH, ts_dsm_debug_show, NULL);
static DEVICE_ATTR(touch_gesture_wakeup_position, S_IRUSR|S_IWUSR, ts_gesture_wake_position_show, NULL);
...
static struct attribute *ts_attributes[] = {
	&dev_attr_touch_chip_info.attr,
       ...
       &dev_attr_touch_gesture_wakeup.attr,
       &dev_attr_dsm_debug.attr,
       &dev_attr_touch_gesture_wakeup_position.attr,
...
static int fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	...
	if (*blank == FB_BLANK_UNBLANK) {
		...
		if (TS_WORK == atomic_read(&g_ts_data.state)||(TS_WORK_IN_SLEEP == atomic_read(&g_ts_data.state))) {
			/*suspend device*/
			switch(event) {
			case FB_EARLY_EVENT_BLANK:
				...
				break;
			case FB_EVENT_BLANK:
				error = ts_power_control_notify(TS_SUSPEND_DEVICE, SHORT_SYNC_TIMEOUT);	/*touch power off*/
				if (error)
...
static inline void ts_report_input(struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	if(((g_ts_data.chip_data->rs_info.sleep_mode == TS_GESTURE_MODE) ||
		(g_ts_data.chip_data->rs_info.gesture_palm_set_flag == true)) &&
	...
	struct ts_cmd_node *in_cmd, struct ts_cmd_node *out_cmd)
{
	...
	if(g_ts_data.chip_data->rs_info.sleep_mode == TS_POWER_OFF_MODE) {
		switch (pm_type) {
	...
	} else if (g_ts_data.chip_data->rs_info.sleep_mode == TS_GESTURE_MODE) {
		switch (pm_type) {
		case TS_BEFORE_SUSPEND: 		/*do something before suspend*/
			ts_stop_wd_timer(&g_ts_data);
			disable_irq(irq_id);
...
static int ts_init(void)
{
	...
	/*use easy_wake_up mode, set device TP wake_up source*/
	device_init_wakeup(&(g_ts_data.input_dev->dev), 1);
