...
#ifdef HW_OOB
	irq_flags =
		irq_flags | IORESOURCE_IRQ | IORESOURCE_IRQ_HIGHLEVEL | IORESOURCE_IRQ_SHAREABLE | IRQF_NO_SUSPEND;
#else
	 irq_flags = irq_flags | IRQF_TRIGGER_FALLING;
#endif /* HW_OOB */
	...
	if (sdhcinfo->oob_irq_registered && sdhcinfo->oob_irq_enable_flag != flag) {
		...
		if (flag) {
			...
			if (device_may_wakeup(sdhcinfo->dev))
				...
				if (error)
					SDLX_MSG(("%s disable_irq_wake error=%d \n", __FUNCTION__, error));
