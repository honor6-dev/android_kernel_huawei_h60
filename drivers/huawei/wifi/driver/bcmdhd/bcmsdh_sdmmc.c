...
sdioh_sdmmc_card_regread(sdioh_info_t *sd, int func, uint32 regaddr, int regsize, uint32 *data)
{
	...
	if ((func == 0) || (regsize == 1)) {
		...
		status = sdioh_request_byte(sd, SDIOH_READ, func, regaddr, &temp);
		if(status == SDIOH_API_RC_FAIL)
		{
			sd_data(("%s: sdioh_request_byte failed\n", __FUNCTION__));
			return ERROR;
		}
		*data = temp;
		*data &= 0xff;
		...
		status = sdioh_request_word(sd, 0, SDIOH_READ, func, regaddr, data, regsize);
		if(status == SDIOH_API_RC_FAIL)
		{
			sd_data(("%s: sdioh_request_word failed\n", __FUNCTION__));
			return ERROR;
		}
		if (regsize == 2)
			*data &= 0xffff;
...
void sdioh_power_off(void)
{
	struct mmc_host *host = NULL;

	if ((gInstance->func[0]!=NULL)&&(gInstance->func[0]->card!=NULL)&&(gInstance->func[0]->card->host!=NULL)) {
		host = gInstance->func[0]->card->host;
	}else{
		sd_err(("%s: Failed to power off sdio host\n",__FUNCTION__));
		return;
	}

	sdio_claim_host(gInstance->func[0]);
	mmc_power_save_host_for_wifi(host);
	sdio_release_host(gInstance->func[0]);
}



void sdioh_power_on(void)
{
	struct mmc_host *host = NULL;
	...
	if ((gInstance->func[0]!=NULL)&&(gInstance->func[0]->card!=NULL)&&(gInstance->func[0]->card->host!=NULL)) {
		...
		sd_err(("%s: Failed to power on sdio host\n",__FUNCTION__));
		return;
	}

	sdio_claim_host(gInstance->func[0]);
	mmc_power_restore_host_for_wifi(host);
	sdio_release_host(gInstance->func[0]);
}
