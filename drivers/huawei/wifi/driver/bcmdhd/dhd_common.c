...
{
	if(strlen(g_nvram_name) != 0 && strlen(g_fw_name) != 0){
		snprintf(nvram_path, MOD_PARAM_PATHLEN, PRODUCT_NVRAM_PATH, g_nvram_name);
		snprintf(firmware_path,MOD_PARAM_PATHLEN, PRODUCT_FIRMWARE_PATH, g_fw_name);
		bcm_strncpy_s(fw_path, sizeof(fw_path), firmware_path, MOD_PARAM_PATHLEN-1);
		bcm_strncpy_s(nv_path, sizeof(nv_path), nvram_path, MOD_PARAM_PATHLEN-1);
		HW_PRINT((WIFI_TAG"firmware name:%s, nvram name:%s\n", firmware_path, nvram_path));
	}else{
		HW_PRINT((WIFI_TAG"get firmware & nvram from dts error\n"));
