...
wl_cfgp2p_disable_discovery(struct wl_priv *wl)
{
	...
	if(!wl->p2p){
		printk(KERN_ERR "%s:p2p has been freed. do nothing!\n", __FUNCTION__);
		return BCME_ERROR;
	}
...
wl_cfgp2p_listen_expired(unsigned long data)
{
	...
	CFGP2P_DBG((" Enter\n"));
	if(wl == NULL || wl->p2p == NULL){
		CFGP2P_ERR(("wl or wl->p2p is null, just return"));
		return;
	}
...
#define EXTRA_DELAY_TIME	100
	...
	if (!p2p_is_on(wl)) {
		CFGP2P_ERR(("p2p_is_on is false, goto exit\n"));
		ret = BCME_ERROR;
		goto exit;
	}
	if (timer_pending(&wl->p2p->listen_timer)) {
	...
	if(wl->p2p){
		...
		CFGP2P_ERR((" wl->p2p is released, so we have noting to do\n"));
		ret = BCME_ERROR;
	}
...
wl_cfgp2p_set_p2p_noa(struct wl_priv *wl, struct net_device *ndev, char* buf, int len)
{
	...
	if (wl->p2p && wl->p2p->vif_created) {
		...
		ret = sscanf(buf, "%10d %10d %10d", &count, &start, &duration);
		if(ret < 0)
...
wl_cfgp2p_set_p2p_ps(struct wl_priv *wl, struct net_device *ndev, char* buf, int len)
{
	...
	CFGP2P_DBG((" Enter\n"));
	if (wl->p2p && wl->p2p->vif_created) {
		ret = sscanf(buf, "%10d %10d %10d", &legacy_ps, &ps, &ctw);
		if(ret < 0)
...
wl_cfgp2p_add_p2p_disc_if(void)
{
	...
	if(!(wl->p2p)){
		printk(KERN_ERR "%s, %d: p2p is null.", __FUNCTION__, __LINE__);
		return NULL;
	}
	...
	if (wl->p2p_wdev) {
#if defined(WL_NEWCFG_PRIVCMD_SUPPORT)
		CFGP2P_ERR(("p2p_wdev defined already, delete the old one.\n"));
		wl_cfgp2p_del_p2p_disc_if(wl->p2p_wdev);   // reset p2p_wdev  for new
#else
...
#if defined(WL_NEWCFG_PRIVCMD_SUPPORT)
	if(wl->p2p_net) {
		memcpy(wl->p2p_net->dev_addr, &wl->p2p->dev_addr, ETHER_ADDR_LEN);
	}
#endif
...
wl_cfgp2p_stop_p2p_device(struct wiphy *wiphy, struct wireless_dev *wdev)
{
	...
	if (!g_wifi_on) {
		CFGP2P_ERR(("%s: Ignore cmd NL80211_CMD_STOP_P2P_DEVICE - iface wlan0 is down\n", __FUNCTION__));
		return;
	}
...
wl_cfgp2p_del_p2p_disc_if(struct wireless_dev *wdev)
{
	...
	mutex_lock(&wl->unreg_wdev_sync);

	if (!wdev)
	{
		mutex_unlock(&wl->unreg_wdev_sync);
		return -EINVAL;
	}
	...
	{
		mutex_unlock(&wl->unreg_wdev_sync);
		CFGP2P_ERR(("P2P interface already unregistered\n"));
		return 0;
	}
