 ...
 *
 * $Id: dhd_wlfc.c 432623 2013-10-29 03:01:49Z $
 *
 */
 ...
 * 3. Two lock involved to protect different resource. dhd_os_wlfc_pub_block is to protect resources
 *    in dhd_pub_t structure and dhd_os_wlfc_block is to protect all other wlfc resouces.
 * 4. If one function needs both locks, take dhd_os_wlfc_block first and then dhd_os_wlfc_pub_block.
 * 5. Non-Proptxstatus module call public functions only and avoid wlfc lock operation.
 *
 */
...
_dhd_wlfc_prec_drop(dhd_pub_t *dhdp, int prec, void* p, bool countdown)
{
	...
	if ((dhdp->wlfc_mode == WLFC_MODE_HANGER) && (prec & 1)) {
	...
	wlfc_mac_descriptor_t* entry, void* p, int header_needed, uint32* slot)
{
	...
	*slot = hslot;
	...
	if (header_needed) {
		if (dhdp->wlfc_mode == WLFC_MODE_HANGER) {
			hslot = _dhd_wlfc_hanger_get_free_slot(ctx->hanger);
		} else if (dhdp->wlfc_mode == WLFC_MODE_AFQ) {
	...
	if (rc == BCME_OK) {
		...
		if ((dhdp->wlfc_mode == WLFC_MODE_HANGER) && header_needed) {
			/*
	...
	*ac_credit_spent = ((prec == AC_COUNT) && !ctx->bcmc_credit_supported) ? 0 : 1;
	...
	for (i = 0; i < total_entries; i++) {
		if (only_no_credit) {
			...
			/* move head inside queue to ensure fair round-robin */
...
_dhd_wlfc_cleanup_txq(dhd_pub_t *dhd, f_processpkt_t fn, void *arg)
{
	...
	for (prec = 0; prec < txq->num_prec; prec++) {
		while ((pkt = _dhd_wlfc_pktq_pdeq_with_fn(txq, prec, fn, arg))) {
			if ((dhd->wlfc_mode == WLFC_MODE_HANGER) &&
	...
	*/
	...
	for (i = 0; i < total_entries; i++) {
		if (table[i].occupied) {
			...
			if ((dhd->wlfc_mode == WLFC_MODE_AFQ) && (table[i].afq.len)) {
	...
	f_processpkt_t fn, void *arg)
{
	...
	if ((action == eWLFC_MAC_ENTRY_ACTION_ADD) || (action == eWLFC_MAC_ENTRY_ACTION_UPDATE)) {
		...
		if (action == eWLFC_MAC_ENTRY_ACTION_ADD) {
			...
			if (dhdp->wlfc_mode == WLFC_MODE_AFQ) {
...
_dhd_wlfc_FIFOcreditmap_update(void* state, uint8* credits)
{
	...
	if (ctx->creditmap_received)
		return BCME_OK;

	ctx->creditmap_received = TRUE;
...
_dhd_wlfc_compressed_txstatus_update(dhd_pub_t *dhd, uint8* pkt_info, uint8 len)
{
	...
	while (count < len) {
		...
		} else if (dhd->wlfc_mode == WLFC_MODE_AFQ) {
...
bool dhd_wlfc_is_supported(dhd_pub_t *dhd, bool bAcquireLock)
{
	bool rc = TRUE;
	...
	*/
	...
	for (ac = AC_COUNT; ac >= 0; ac--) {
		...
		while (FALSE == dhdp->proptxstatus_txoff) {
			 ...
			 */
			only_no_credit = (ctx->FIFO_credit[ac] < 1);
			if ((ac == AC_COUNT) && !ctx->bcmc_credit_supported) {
				only_no_credit = FALSE;
			}
...
dhd_wlfc_txcomplete(dhd_pub_t *dhd, void *txp, bool success)
{
	...
	if (!success) {
		...
		if (dhd->wlfc_mode == WLFC_MODE_AFQ) {
...
dhd_wlfc_init(dhd_pub_t *dhd)
{
	...
	*/
	...
	if (ret < 0) {
		dhd->wlfc_mode = WLFC_MODE_HANGER;
	} else {
		dhd->wlfc_mode = mode;
	}
	DHD_ERROR(("dhd_wlfc_init(): wlfc_mode=%d, ret=%d\n", dhd->wlfc_mode, ret));
...
dhd_wlfc_dump(dhd_pub_t *dhdp, struct bcmstrbuf *strbuf)
{
	...
	bcm_bprintf(strbuf, "PKTS (credit,sent,drop,outoforder) "
		"(AC0[%d,%d,%d,%d,%d],AC1[%d,%d,%d,%d,%d],AC2[%d,%d,%d,%d,%d],"
		"AC3[%d,%d,%d,%d,%d],BC_MC[%d,%d,%d,%d,%d])\n",
	...
	for (i = 0; i < WLFC_MAX_IFNUM; i++) {
		if (interfaces[i].occupied) {
			...
			bcm_bprintf(strbuf, "INTERFACE[%d].PSQ"
				"(ac0,sup0,afq0),(ac1,sup1,afq1),(ac2,sup2,afq2),(ac3,sup3,afq3) = "
