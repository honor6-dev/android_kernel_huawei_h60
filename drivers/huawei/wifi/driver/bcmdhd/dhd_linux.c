...
#include "hw_wifi.h"
#ifndef CONFIG_V9R1_FREQ_CTRL_FLAG
#define CONFIG_V9R1_FREQ_CTRL_FLAG
#endif

#ifdef  CONFIG_V9R1_FREQ_CTRL_FLAG
...
#define NUMBIN 34
extern void wl_reset_ap_mac_status_list(void);
...
#define WLAN_REASON_RECOVERY 88
#endif
#endif
...
static char dhd_version[] = "Dongle Host Driver, version " EPI_VERSION_STR HUAWEI_VERSION_STR
#ifdef DHD_DEBUG
...
#ifndef SUPPORT_PM2_ONLY
				if(dhd_wl_ioctl_cmd(dhd, WLC_SET_PM, (char *)&power_mode,
				                 sizeof(power_mode), TRUE, 0) < 0)
				{
					 DHD_ERROR(("%s: read Set PM. \n", __FUNCTION__));
				}
#endif /* SUPPORT_PM2_ONLY */
...
#ifdef PROP_TXSTATUS
	if (dhd_wlfc_is_supported(dhdp, TRUE)) {
...
#ifdef  CONFIG_V9R1_FREQ_CTRL_FLAG
		/*record send count include 802.11 MAC header length and IP packet total length.*/
		v9r1_tx_rx_counter(datalen);
#endif
...
int dhd_ioctl_process(dhd_pub_t *pub, int ifidx, dhd_ioctl_t *ioc)
{
	...
	if(bcmerror < 0)
		DHD_ERROR(("%s:dhd_wl_ioctl error \n",__FUNCTION__));
done:
...
dhd_open(struct net_device *net)
{
...
#ifdef  CONFIG_V9R1_FREQ_CTRL_FLAG
	HW_PRINT((WIFI_TAG"v9r1_freq_ctrl_init, init enter\n"));
	v9r1_freq_ctrl_init();
#endif
	...
	if (strlen(firmware_path) != 0) {
		...
		if(strlen(g_fw_name) != 0 && (NULL == strstr(firmware_path, "apsta"))){ //station firmware
			snprintf(firmware_path,MOD_PARAM_PATHLEN, PRODUCT_FIRMWARE_PATH, g_fw_name);
			strncpy(fw_path, firmware_path, sizeof(fw_path)-1);
			HW_PRINT((WIFI_TAG"%s: station firmware\n", __FUNCTION__));
		}else{ //test or softap firmware
			strncpy(fw_path, firmware_path, sizeof(fw_path)-1);
			HW_PRINT((WIFI_TAG"%s: test or softap firmware\n", __FUNCTION__));
		}
		firmware_path[0] = '\0';
	}
...
exit:
	...
	if (ret){
		...
		hw_wifi_dsm_client_notify("open file failed in dhd_open");
...
#endif
#endif /* MULTIPLE_SUPPLICANT */
	HW_PRINT((WIFI_TAG"fw_path:%s, nv_path:%s\n", fw_path, nv_path));
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	if (iface_name[0]) {
		...
		{
			DHD_ERROR(("dhd_attach: WARNING iface_name wrong %s\n", iface_name));		    
			strncat(net->name, "%d", sizeof("%d"));
		}
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	uint bcn_timeout = hw_get_bcn_timeout();
	uint retry_max = 3;
...
#ifdef ROAM_ENABLE
	uint roamvar = 0;
	int roam_trigger_2G[2] = {CUSTOM_ROAM_TRIGGER_SETTING, WLC_BAND_2G};
	int roam_trigger_5G[2] = {HW_5G_CUSTOM_ROAM_TRIGGER_SETTING, WLC_BAND_5G};
	...
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER, roam_trigger_2G,
		sizeof(roam_trigger_2G), TRUE, 0)) < 0)
		DHD_ERROR(("%s: roam trigger set failed %d\n", __FUNCTION__, ret));
	if ((ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_ROAM_TRIGGER, roam_trigger_5G,
		sizeof(roam_trigger_5G), TRUE, 0)) < 0)
		DHD_ERROR(("%s: roam trigger set failed %d\n", __FUNCTION__, ret));
	...
	if((dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0 )
		 DHD_ERROR(("%s: roam scan period set failed \n", __FUNCTION__));
#endif
#ifdef WLTDLS
	...
	/* Set PowerSave mode */
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_PM, (char *)&power_mode, sizeof(power_mode), TRUE, 0) < 0)
		 DHD_ERROR(("%s: set powersave mode failed\n", __FUNCTION__));

	/* Match Host and Dongle rx alignment */
	bcm_mkiovar("bus:txglomalign", (char *)&dongle_align, 4, iovbuf, sizeof(iovbuf));
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0)< 0)
		 DHD_ERROR(("%s: Match Host and Dongle rx alignment failed \n", __FUNCTION__));

	/* huawei delete for coverity
	...
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0)
		 DHD_ERROR(("%s: Setup timeout if Beacons are lost and roam is off to report link down failed \n", __FUNCTION__));
	/* Setup assoc_retry_max count to reconnect target AP in dongle */
	bcm_mkiovar("assoc_retry_max", (char *)&retry_max, 4, iovbuf, sizeof(iovbuf));
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0)
		DHD_ERROR(("%s:Setup assoc_retry_max count to reconnect target AP in dongle failed \n", __FUNCTION__));
#if defined(AP) && !defined(WLP2P)
	/* Turn off MPC in AP mode */
	bcm_mkiovar("mpc", (char *)&mpc, 4, iovbuf, sizeof(iovbuf));
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0)
		DHD_ERROR(("%s: Turn off MPC in AP mode failed \n", __FUNCTION__));
	bcm_mkiovar("apsta", (char *)&apsta, 4, iovbuf, sizeof(iovbuf));
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0)
		DHD_ERROR(("%s: Turn off apsta in AP mode failed \n", __FUNCTION__));
#endif /* defined(AP) && !defined(WLP2P) */


#if defined(SOFTAP)
	if (ap_fw_loaded == TRUE) {
		if(dhd_wl_ioctl_cmd(dhd, WLC_SET_DTIMPRD, (char *)&dtim, sizeof(dtim), TRUE, 0) < 0 )
			DHD_ERROR(("%s: ap fw load failed \n", __FUNCTION__));
	}
#endif 
	...
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_SCAN_CHANNEL_TIME, (char *)&scan_assoc_time,
		sizeof(scan_assoc_time), TRUE, 0) < 0)
		DHD_ERROR(("%s Set scan channel time failed\n", __FUNCTION__));
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_SCAN_UNASSOC_TIME, (char *)&scan_unassoc_time,
		sizeof(scan_unassoc_time), TRUE, 0))
		DHD_ERROR(("%s Set scan unassoc time failed\n", __FUNCTION__));
	if(dhd_wl_ioctl_cmd(dhd, WLC_SET_SCAN_PASSIVE_TIME, (char *)&scan_passive_time,
		sizeof(scan_passive_time), TRUE, 0) < 0)
		DHD_ERROR(("%s Set scan passive time failed\n", __FUNCTION__));
...
dhd_module_init(void)
{
	 ...
	 */
	if (!g_wifi_poweron) {
		printf("%s: wifi_set_power() failed\n", __FUNCTION__);
		error = -ENODEV;
		goto fail_1;
	}
#endif /* defined(CONFIG_WIFI_CONTROL_FUNC) */
...
fail_0:
    ...
    if(0 != error)
    {
        set_wifi_open_err_code(error);
        hw_wifi_dsm_client_notify("init wifi driver failed in dhd_module_init");
    }
	return error;
}
...
dhd_os_wd_timer(void *bus, uint wdtick)
{
	...
	if (!wdtick && dhd->wd_timer_valid == TRUE) {
		...
		del_timer_sync(&dhd->timer);
#else
		if(del_timer(&dhd->timer) == 0)
			DHD_TRACE(("%s: del_timer failed.\n",__FUNCTION__));
#endif /* DHDTHREAD */
		DHD_OS_WD_WAKE_UNLOCK(pub);
		return;
	}
	...
	while (ntimes && pend) {
		if (pend) {
			...
			if(schedule_timeout(timeout))
				DHD_TRACE(("%s:schedule timeout failed.\n",__FUNCTION__));
			set_current_state(TASK_RUNNING);
			ntimes--;
		}
...
void dhd_set_version_info(dhd_pub_t *dhdp, char *fw)
{
	...
	snprintf(&info_string[i], sizeof(info_string) - i,
...
int dhd_reopen_dev(struct net_device *dev)
{
	...
	if(dhd->pub.op_mode == DHD_FLAG_HOSTAP_MODE) {
		...
		DHD_ERROR(("do recovery, dhd reopen\n"));
		dhd_stop(dev);
		dhd_open(dev);
		wl_cfg80211_hang(dev, WLAN_REASON_RECOVERY);
	}
	return 0;
}
#endif
...
int dhd_wifi_shutdown(void)
{
    ...
    DHD_ERROR(("##> %s enter\n", __FUNCTION__));
    dhd_pub = bcmsdh_get_drvdata();
    
    if (dhd_pub && dhd_os_check_if_up(dhd_pub)) 
    {
        ...
        {
            ret = dhd_stop(dhd_if->net);
        }
    }

    DHD_ERROR(("##> %s exit, ret=%d\n", __FUNCTION__, ret));
