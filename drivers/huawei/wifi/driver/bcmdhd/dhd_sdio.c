...
static void wifi_apr_log_worker(struct work_struct *dummy)
{
	char cmd[256];
	char time_buf[16];
	int  lret;
	...
	snprintf(cmd, 256, "%s%s%s%s",
	"archive -i /data/android_logs/kmsgcat-log -i /data/android_logs/kmsgcat-log.1",
	" -o ", time_buf, "_WiFi -z zip");

	lret = log_to_exception("wifi", cmd);
	if(lret < 0 )
		DHD_ERROR(("logexception sysfs err.\n"));
}

/* create wifi apr log worker,
 * caller decide to schedule work right now or delay,
 * NOTICE:wifi apr log work should work in process context and NOT RT process context
 */
static DECLARE_DELAYED_WORK(wifi_apr_log_wk, wifi_apr_log_worker);
...
dhd_bus_txdata(struct dhd_bus *bus, void *pkt)
{
	...
	if (dhd_deferred_tx || bus->fcstate || pktq_len(&bus->txq) || bus->dpc_sched ||
		...
		ret = dhdsdio_txpkt(bus, pkt, chan, TRUE, FALSE);
...
dhdsdio_write_vars(dhd_bus_t *bus)
{
	...
	if (bus->vars) {
		...
		if (!vbuffer)
		{
			DHD_ERROR(("%s:vbuffer malloc failed.\n", __FUNCTION__));
			return BCME_NOMEM;
		}
		...
		if (!nvram_ularray)
		{
			DHD_ERROR(("%s:nvram_ularray malloc failed.\n", __FUNCTION__));
			return BCME_NOMEM;
		}
	...
	if (bus->glom) {
		...
		if (errcode < 0) {
			...
			if (bus->glomerr++ < 3) {
				...
				/* if sdio glom rx fail,will send NAK several times.
				 * so just send apr log right now once meet
				 * sdio bus error after retry
				 * NOTICE:only report one time apr log in each time
				 */
				if (errcode == BCME_SDIO_ERROR && apr_report_cnt == 0)
				{
					schedule_delayed_work(&wifi_apr_log_wk, 0);
					apr_report_cnt++;
				}
...
dhd_bus_watchdog(dhd_pub_t *dhdp)
{
	...
	/* Ignore watchdog sdio R/W operation when system enter suspend routine,
	 * will recover after cfg80211 resume
	 */
	if (cfg80211_suspend == TRUE)
		return FALSE;
...
dhd_bus_console_in(dhd_pub_t *dhdp, uchar *msg, uint msglen)
{
	...
	if ((pkt = PKTGET(bus->dhd->osh, 4 + SDPCM_RESERVE, TRUE)) != NULL)
		if(dhdsdio_txpkt(bus, pkt, SDPCM_EVENT_CHANNEL, TRUE, FALSE))
			DHD_INFO(("dhdsdio_txpkt failed\n"));
	...
	uint16 func, uint bustype, void *regsva, osl_t * osh, void *sdh)
{
	int ret;
	dhd_bus_t *bus;
#if 0//def GET_CUSTOM_MAC_ENABLE
