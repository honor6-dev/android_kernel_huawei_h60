...
u32 wl_dbg_level = WL_DBG_ERR | WL_DBG_SCAN;

/* cfg80211 suspend flag */
volatile bool cfg80211_suspend = FALSE;

#ifdef SCAN_TIMEOUT_FIX
#define MAX_SCANTIMEOUT_COUNT 2
int	scan_timeout_counter = 0;
#endif
...
static s32 wl_prune_event_handler(struct wl_priv *wl, bcm_struct_cfgdev *cfgdev,
	const wl_event_msg_t *e, void *data);

static s32 wl_check_assoc_status(struct wl_priv *wl, struct net_device *ndev,
	const wl_event_msg_t *e);	

static s32 wl_clear_wrong_action_handle(struct wl_priv *wl, struct net_device *ndev, wl_extjoin_params_t *ext_join_params);
#endif
...
#ifdef PROP_TXSTATUS_VSDB
			dhd_wlfc_get_enable(dhd, &enabled);
		if (enabled && wl->wlfc_on && dhd->op_mode != DHD_FLAG_HOSTAP_MODE &&
			dhd->op_mode != DHD_FLAG_IBSS_MODE) {
			dhd_wlfc_deinit(dhd);
			wl->wlfc_on = false;
		}
		...
		if (wl->p2p->vif_created) {
			...
			if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
				...
				if (ret == 0) {
					...
					timeout = wait_for_completion_timeout(&wl->iface_disable,
			...
			*/
			...
			if(wl->p2p){
				...
				timeout = wait_event_interruptible_timeout(wl->netif_change_event,
					(wl->p2p->vif_created == false),
					msecs_to_jiffies(MAX_WAIT_TIME));
				if (wl->p2p->vif_created == false) {
					WL_DBG(("IFDEL operation done, timeout = %d\n", timeout));
				} else {
					WL_ERR(("IFDEL didn't complete properly, timeout  = %d\n", timeout));
				}
			}
		...
		if (unlikely(err)) {
			if (err == BCME_EPERM) {
				...
				if (dhd->op_mode & DHD_FLAG_P2P_MODE) {
					WL_ERR(("Disable scan suppress in sta/p2p mode \n"));
					ret = wldev_ioctl(ndev, WLC_SET_SCANSUPPRESS, &scan_suppress, sizeof(int), true);
					if (unlikely(ret)) {
...
#ifdef WRONG_ACTION_PATCH
	wl_clear_wrong_action_handle(wl, dev, ext_join_params);
#endif
	...
	u8 *mac, struct station_info *sinfo)
{
	...
	if (wl_get_mode_by_netdev(wl, dev) == WL_MODE_AP) {
		...
		WL_INFO(("sta->idle = %d, sta->in = %d\n", dtoh32(sta->idle), dtoh32(sta->in)));
		sta->idle = 10; /*fix to 10s*/
...
static s32 wl_cfg80211_resume(struct wiphy *wiphy)
{
	...
	cfg80211_suspend = FALSE;

	HW_PRINT((WIFI_TAG"Enter: %s\n", __FUNCTION__));
	HW_PRINT((WIFI_TAG"cfg80211_suspend: %d\n", cfg80211_suspend));
...
wl_cfg80211_af_searching_channel(struct wl_priv *wl, struct net_device *dev)
{
	...
	while ((wl->afx_hdl->retry < max_retry) &&
		...
		timeout =  wait_for_completion_timeout(&wl->act_frm_scan,
	...
	wl_action_frame_t *action_frame, u16 action_frame_len, s32 bssidx)
{
	...
	if (config_af_params.search_channel) {
		...
		if(err < 0)
			WL_ERR(("P2P is discover is failed. Maybe discovery is off;\n"));
		/* update channel */
	...
	if (ack && config_af_params.extra_listen &&
		...
		if (extar_listen_time > 50) {
			...
			if (wl_cfgp2p_discover_listen(wl, wl->af_sent_channel,
				...
				timeout = wait_for_completion_timeout(&wl->wait_next_af,
...
/*add to avoid NULL pointer panic begin*/
	if (NULL == dev){
		WL_ERR(("%s: dev is NULL\n", __FUNCTION__));
		return BCME_ERROR;
	}
/*add to avoid NULL pointer panic end*/
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (wl_get_mode_by_netdev(wl, ndev) == WL_MODE_AP) {
		...
		if (event == WLC_E_ASSOC) {
            wl_check_assoc_status(wl, ndev, e);
		}
#endif		
...
#endif /* WLTDLS */
#ifdef WRONG_ACTION_PATCH
    wl->evt_handler[WLC_E_PRUNE] = wl_prune_event_handler;
#endif
}
		...
		case NETDEV_DOWN:
		{
			...
			while (work_pending(&wdev->cleanup_work)) {
				...
				if(schedule_timeout(100))
					WL_ERR(("schedule_timeout failed.\n"));
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (status == WLC_E_STATUS_PARTIAL) {
			...
			for (i = 0; i < list->count; i++) {
				...
				if (!bcmp(&bi->BSSID, &bss->BSSID, ETHER_ADDR_LEN) &&
					...
					*/
					...
					WL_INFO(("%s("MACDBG"), i=%d prev: RSSI %d"
					...
					if (dtoh32(bss->length) != bi_length) {
						...
						WL_INFO(("bss info replacement"
							" is occured(bcast:%d->probresp%d)\n",
							bss->ie_length, bi->ie_length));
						WL_INFO(("%s("MACDBG"), replacement!(%d -> %d)\n",
						bss->SSID, MAC2STRDBG(bi->BSSID.octet),
						prev_len, bi_length));
		...
		{
			struct net_device *pdev = wl_to_prmry_ndev(wl);
			WL_ERR(("SCAN TIMEOUT lead to HANG Notification sent to %s\n", pdev->name));
			net_os_send_hang_message(pdev);
		}
#endif
	} else {
...
#endif /* WL_CFG80211_P2P_DEV_IF || WL_NEWCFG_PRIVCMD_SUPPORT */
...
static s32 wl_event_handler(void *data)
{
	...
	while (down_interruptible (&tsk->sema) == 0) {
		...
		while ((e = wl_deq_event(wl))) {
			...
			struct net_device *cur_netdevice = dhd_idx2net((struct dhd_pub *)(wl->pub),e->emsg.ifidx);
				if(cur_netdevice == NULL)
					return 0;
				cfgdev = ndev_to_wdev(cur_netdevice);
			}
...
static s32
wl_check_assoc_status(struct wl_priv *wl, struct net_device *ndev,
	const wl_event_msg_t *e)
{
	u32 reason = ntoh32(e->reason);
	dhd_pub_t *dhd;
...
static s32 
wl_prune_event_handler(struct wl_priv *wl, bcm_struct_cfgdev *cfgdev,
	const wl_event_msg_t *e, void *data)
{
	struct net_device *ndev = NULL;
	u32 reason = ntoh32(e->reason);
...
static s32 
wl_clear_wrong_action_handle(struct wl_priv *wl, struct net_device *cfgdev, wl_extjoin_params_t *ext_join_params)
{
	struct net_device *ndev = NULL;
	dhd_pub_t *dhd;
