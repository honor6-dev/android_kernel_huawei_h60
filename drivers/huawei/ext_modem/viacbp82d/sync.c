...
void VIA_trigger_signal(int i_on)
{
	/*TODO: wake the sdio host*/
}
...
static irqreturn_t asc_irq_cp_indicate_state(int irq, void *data)
{
    ...
    hwlog_debug("Irq %s cp_indicate_ap %s.\n", cfg->name, (level == cfg->polar)?"WAKEN":"SLEEP");
    if(level == cfg->polar){
        asc_tx_event_send(tx, AP_TX_EVENT_CP_READY);
        hwlog_info("%s: CP_READY send asc_irq_cp_indicate_state level = %d.\n", __func__, level);
    }else{
        /*do not care*/
        if (AP_TX_ST_SLEEP != atomic_read(&tx->state)) {
            hwlog_info("%s: warning tx->state !=  AP_TX_ST_SLEEP when cp_ready = 1\n", __func__);
            asc_tx_event_send(tx, AP_TX_EVENT_STOP);
        }
        hwlog_info("%s: CP_READY GPIO goto HIGH. level = %d\n", __func__, level);
    }
...
static irqreturn_t asc_irq_cp_wake_ap(int irq, void *data)
{
    ...
    hwlog_debug("Irq %s cp_wake_ap, requset ap to be %s.\n", cfg->name, (level == cfg->polar)?"WAKEN":"SLEEP");
...
static int asc_rx_event_send(struct asc_rx_handle *rx, int id)
{
    ...
    if(rx->thread == NULL){
        hwlog_err("%s:no thread for event\n", __FUNCTION__);
        return ret;
    }
...
//    daemonize(name);
    hwlog_debug("%s thread start now.\n", name);
...
{
    struct asc_rx_handle *rx = (struct asc_rx_handle *)data;
    //hwlog_debug("%s timer is  time out.\n", rx->name);
...
static int asc_rx_handle_init(struct asc_rx_handle *rx)
{
    ...
    if(cfg->gpio_ready >= 0){
        ...
        if(ret < 0){
            hwlog_err("Fail to requset ap_ready gpio %d for %s.\n", cfg->gpio_ready, cfg->name);
    ...
    if(cfg->gpio_wake >= 0){
        ...
        if(ret < 0){
            hwlog_err("Fail to requset cp_wake_ap gpio %d for %s.\n", cfg->gpio_wake, cfg->name);
        ...
        oem_gpio_set_irq_type(cfg->gpio_wake, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING);
        ret = oem_gpio_request_irq(cfg->gpio_wake, asc_irq_cp_wake_ap, IRQF_SHARED | IRQF_NO_SUSPEND|IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "cp_wake_ap", rx);
        oem_gpio_irq_unmask(cfg->gpio_wake);
        if (ret < 0) {
            hwlog_err("fail to request cp_wake_ap irq for %s\n", cfg->name);
    ...
    if(!name){
        ret = -ENOMEM;
        hwlog_err("%s: no memory to malloc for wake lock name\n", __FUNCTION__);
    ...
    kthread_run(asc_rx_event_thread, rx, "asc_rx_spi");
...
static int asc_rx_handle_sleep(void *data, int event)
{
    ...
    switch(event){
        case AP_RX_EVENT_REQUEST:
            ...
            break;
         default:
            hwlog_debug("ignore the rx event %d in state(%s)", event, rx->table[atomic_read(&rx->state)].name);
    }
...
{
    hwlog_info("%s  need set %d set ap_wake_cp wake\n", __func__, tx->cfg.gpio_wake);
...
{
    hwlog_info("%s  need set %d set ap_wake_cp sleep\n", __func__, tx->cfg.gpio_wake);
    if(tx->cfg.gpio_wake >= 0)
        oem_gpio_direction_output(tx->cfg.gpio_wake, !tx->cfg.polar);
}
...
static int asc_tx_event_send(struct asc_tx_handle *tx, int id)
{
    ...
    if(id >= 0){
        ...
        /*insert a new event to the list tail and wakeup the process thread*/
        //hwlog_debug("Send tx event(%d)  to state(%s).\n", id, tx->table[atomic_read(&tx->state)].name);
        event->id = id;
        spin_lock_irqsave(&tx->slock, flags);
...
//    daemonize(name);
    ...
    while(1){
        ...
        dsp = tx->table + index;
        if(dsp->handle){
            hwlog_info("Tx(%s): process event(%d) in state(%s).\n", tx->cfg.name, id, dsp->name);
            dsp->handle(tx, id);
            hwlog_info("Tx(%s): go into state(%s) .\n", tx->cfg.name, tx->table[atomic_read(&tx->state)].name);
        }
...
{
    struct asc_tx_handle *tx = (struct asc_tx_handle *)data;
    //hwlog_debug("%s tx wait ready timer is timeout.\n", tx->name);
...
static void asc_tx_wait_idle_timer(unsigned long data)
{
    ...
    struct asc_tx_handle *tx = (struct asc_tx_handle *)data;
    //hwlog_debug("%s tx wait idle timer is timeout.\n", tx->name);
...
{
    struct asc_tx_handle *tx = (struct asc_tx_handle *)data;
    //hwlog_debug("%s tx wait sleep timer is timeout.\n", tx->name);
...
static int asc_tx_handle_init(struct asc_tx_handle *tx)
{
    ...
    if(ret < 0){
        hwlog_err("Fail to requset ap_wake_cp gpio %d for %s.\n", cfg->gpio_wake, cfg->name);
    ...
    if(cfg->gpio_ready >= 0){
        ...
        if(ret < 0){
            hwlog_err("Fail to requset cp_ready gpio %d for %s.\n", cfg->gpio_ready, cfg->name);
        ...
        ret = oem_gpio_request_irq(cfg->gpio_ready, asc_irq_cp_indicate_state,
                        IRQF_SHARED|IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING, "cp_indicate_state", tx);
        oem_gpio_irq_unmask(cfg->gpio_ready);
        if (ret < 0) {
            hwlog_err("fail to request irq for %s:cp_ready\n", cfg->name);
    ...
    kthread_run(asc_tx_event_thread, tx, "asc_spi_tx");
        ...
        case AP_TX_EVENT_REQUEST:
            ...
            if(tx->cfg.gpio_ready >= 0){
                ...
                if(asc_tx_cp_be_ready(tx)){
                    ...
                    if(asc_tx_cp_be_ready(tx)){
                        hwlog_debug("Tx:cp %s was ready now.\n", tx->cfg.name);
            ...
            break;
         default:
            hwlog_debug("Tx: ignore event %d in state(%s)", event, tx->table[atomic_read(&tx->state)].name);
    }

    //hwlog_debug("Tx(%s): go into state(%s).\n", tx->name, tx->table[atomic_read(&tx->state)].name);
    return ret;
}
...
static int asc_tx_handle_wait_ready(void *data, int event)
{
    ...
    switch(event){
       ...
       case AP_TX_EVENT_WAIT_TIMEOUT:
            hwlog_info("Tx: %s wait cp ready timeout,  try=%d.\n", tx->cfg.name, tx->wait_try);
            asc_tx_sleep_cp(tx);
...
/**
 * asc_tx_set_sleep - change state to sleep.
 * @name: the config name for the handle
 *
 * return 0 OK, others be error
 */
int asc_tx_set_sleep(const char *name)
{
    ...
    if(ret == AP_TX_ST_READY){
        ret = asc_tx_handle_ready(tx, AP_TX_EVENT_STOP);
    }

    return ret;
}
EXPORT_SYMBOL(asc_tx_set_sleep);
...
int asc_tx_check_ready(const char *name)
{
    ...
    if((ret == AP_TX_ST_READY) && (!tx->sleeping)){
        ret = 1;
    }else{
        ret = 0;
    }
...
int asc_tx_add_user(const char *name, struct asc_infor *infor)
{
    ...
    if(NULL == user){
        ...
        if(!user){
            hwlog_err("No memory to create new user reference.\n");
            ret = -ENOMEM;
            goto error;
        }
        ...
        hwlog_err("%s error: user %s already exist!!\n", __FUNCTION__, infor->name);
        ret = -EINVAL;
    }
error:
    return ret;
...
void asc_tx_del_user(const char *path)
{
    ...
    if (!strncmp(name, ASC_TX_SYSFS_USER, ASC_NAME_LEN - 1) || \
         ...
         hwlog_err("Can not delete reserve user %s\n", path);
         return ;
    }
...
int asc_tx_get_ready(const char *path, int sync)
{
    ...
    if(asc_tx_get_user(tx, name) < 0){
        hwlog_err("%s:tx user name %s is unknow\n", __FUNCTION__, name);
        return -ENODEV;
    }
    hwlog_debug("%s: %s=%d, %s=%d\n", __FUNCTION__,\
    ...
    switch(atomic_read(&tx->state)){
        ...
        case AP_TX_ST_READY:
            if(tx->sleeping){
                hwlog_info("%s: tx state is sleeping\n", __FUNCTION__);
                asc_tx_event_send(tx, AP_TX_EVENT_REQUEST);
            }else if(!strncmp(name, ASC_TX_AUTO_USER, strlen(ASC_TX_AUTO_USER))){
                asc_tx_trig_busy(tx);
            }else{
                ; //do nothing
            }
            break;
        default:
            hwlog_err("Unknow tx state %d\n", atomic_read(&tx->state));
            return -EINVAL;
    }

    if(sync){
        if((AP_TX_ST_READY != atomic_read(&tx->state)) || tx->sleeping){
             do{
                sleep_on(&tx->wait_tx_state);
                if(AP_TX_ST_READY == atomic_read(&tx->state)){
                    break;
                    ...
                    hwlog_err("%s: wait CBP get ready for AP tx FAIL after try 3 times.\n", __func__);
                    ret = -EBUSY;
                    break;
                }
             }while(1);
...
int asc_tx_put_ready(const char *path, int sync)
{
    ...
    if(sync){
        if(AP_TX_ST_SLEEP != atomic_read(&tx->state)){
             sleep_on(&tx->wait_tx_state);
             if(AP_TX_ST_SLEEP != atomic_read(&tx->state)){
                hwlog_err("%s: ask CBP goto sleep FAIL.\n", __func__);
                ret = -EBUSY;
            }
        }
    }
...
#define	ASC_TX_READY_TIMEOUT		(4000)//4s
...
int asc_tx_auto_ready(const char *name, int sync)
{
    ...
    tx = asc_tx_handle_lookup(name);
    if(NULL == tx) {
        hwlog_err("%s:Invalid tx handle\n", __FUNCTION__);
        return -ENODEV;
	}

    user = asc_tx_user_lookup(tx, ASC_TX_AUTO_USER);
    if(!user){
        hwlog_err("%s:Invalid tx user\n", __FUNCTION__);
        return  -ENODEV;
    }
    ...
    if(sync){
        if((AP_TX_ST_READY != atomic_read(&tx->state)) || tx->sleeping){
             do{
                timeout = sleep_on_timeout(&tx->wait_tx_state, msecs_to_jiffies(ASC_TX_READY_TIMEOUT));
                if(AP_TX_ST_READY == atomic_read(&tx->state)){
                    break;
                }else if(try <= ASC_TX_TRY_TIMES){
                    asc_tx_event_send(tx, AP_TX_EVENT_REQUEST);
			...
			//sleep timeout or be waken up but not ready, add a log
			hwlog_err("%s %d:sleep_on_timeout return:%d.\n", __func__, __LINE__, (int)timeout);
                }else{
                    hwlog_err("%s: wakeup CBP ready for AP tx FAIL after try 3 times.\n", __func__);
                    ret = -EBUSY;
                    break;
                }
             }while(1);
...
int asc_rx_confirm_ready(const char *name, int ready)
{
    ...
    rx = asc_rx_handle_lookup(name);
    if(!rx){
        hwlog_debug("%s: name %s is unknow\n", __FUNCTION__, name);
        return -ENODEV;
    }

    hwlog_debug("Rx(%s) confirm ready=%d\n", rx->cfg.name, ready);
...
int asc_rx_check_on_start(const char *name)
{
	...
	rx = asc_rx_handle_lookup(name);
	if(!rx){
        hwlog_err("config %s has not already exist.\n", name);
        return -EINVAL;
    }
	...
	if(level == cfg->polar){
		...
		if(AP_RX_ST_SLEEP == atomic_read(&rx->state)){
            hwlog_debug("Rx(%s):check_on_start--send event AP_RX_EVENT_REQUEST.\n", cfg->name);
            ret = asc_rx_event_send(rx, AP_RX_EVENT_REQUEST);
        }else{
        	hwlog_debug("Rx(%s): check_on_start--send event AP_RX_EVENT_AP_READY.\n", cfg->name);
...
int asc_rx_register_handle(struct asc_config *cfg)
{
    ...
    if(NULL == asc_work_queue){
        hwlog_err("%s: error Asc has not been init\n", __FUNCTION__);
        return -EINVAL;
    }
    ...
    if(cfg->gpio_wake < 0){
        hwlog_err("%s: config %s gpio is invalid.\n", __FUNCTION__, cfg->name);
        return -EINVAL;
    }

    rx = asc_rx_handle_lookup(cfg->name);
    if(rx){
        hwlog_err("config %s has already exist.\n", cfg->name);
        return -EINVAL;
    }

    rx = kzalloc(sizeof(struct asc_rx_handle), GFP_KERNEL);
    if(NULL == rx){
        hwlog_err("No memory to alloc rx handle.\n");
        return -ENOMEM;
    }
    ...
    if(ret < 0){
        hwlog_err("fail to init rx handle %s\n", rx->cfg.name);
        kfree(rx);
        return -EINVAL;
    }
    ...
    if(!rx->kobj){
        hwlog_err("fail to create rx handle %s kobject\n", rx->cfg.name);
        kfree(rx);
        return -ENOMEM;
    }
    ...
    spin_unlock_irqrestore(&hdlock, flags);
    hwlog_debug("Register rx handle %s\n", rx->cfg.name);
...
int asc_tx_register_handle(struct asc_config *cfg)
{
    ...
    if(NULL == tx){
        hwlog_err("Fail to alloc memory for tx handle.\n");
        return -ENOMEM;
    }
    ...
    if(ret < 0){
        hwlog_err("Fail to init tx handle %s.\n", tx->cfg.name);
    ...
    spin_unlock_irqrestore(&hdlock, flags);
    hwlog_debug("Register tx handle %s.\n", tx->cfg.name);
...
static int __init asc_init(void)
{
    ...
    if (ret) {
        hwlog_err("platform_device_register failed\n");
