...
u8 *isp_firmware_addr;
u32 isp_firmware_size;
...
static int isp_res_init(struct device *pdev)
{
	...
	isp_hw_data.phyaddr = (u8 *)(base_array[0]);
	isp_hw_data.mem_size = base_array[1];

	isp_hw_data.viraddr = ioremap_nocache((unsigned long)isp_hw_data.phyaddr, isp_hw_data.mem_size);
...
static void meta_data_handle(struct work_struct *work)
{
	u8 *data_src, *data_dst;
	int index=0;
	hwisp_stream_t* stream_vc = NULL;
	hwisp_buf_t* buf = NULL;
	isp_hw_data_t *isp_hw = container_of(work, isp_hw_data_t, meta_data_work);
	u16 b_gain, g_gain, r_gain;
	u32 expo_line;
	u16 gain;

	if (!isp_hw->meta_running) {
		cam_info("%s isp meta is not running", __func__);
		return;
	}
...
#if 0
	cam_info("%s frame_index=%u result_BV=%d avg_raw_lum=%u expo_time=%u expo_line=%u iso=%u gain=%u)",
		__func__, isp_hw->meta_data.common_info.frame_index,
		isp_hw->meta_data.ae_info.result_BV, isp_hw->meta_data.ae_info.avg_raw_lum,
		isp_hw->meta_data.ae_info.expo_time, isp_hw->meta_data.ae_info.expo_line,
		isp_hw->meta_data.ae_info.iso, isp_hw->meta_data.ae_info.gain);
#endif
}
...
int hw_isp_init(struct device *pdev)
{
	...
	memset(isp_hw_data.mac_array, 0, sizeof(struct ovisp32_mac)*ISP_PORT_MAX);

	/* meta data buffer prepare */
	INIT_WORK(&isp_hw->meta_data_work, meta_data_handle);
	meta_vir_addr = (u8 *)dma_alloc_coherent(pdev, META_DATA_RAW_SIZE, &meta_phy_addr, GFP_KERNEL);
	if (meta_vir_addr && meta_phy_addr) {
		 isp_hw->meta_viraddr = meta_vir_addr;
		 isp_hw->meta_phyaddr = (u8 *)meta_phy_addr;
		 cam_info("%s meta_viraddr=0x%p meta_phyaddr=0x%x size=%u", __func__,
					meta_vir_addr, meta_phy_addr, META_DATA_RAW_SIZE);
	} else {
...
int hw_isp_deinit(struct device *pdev)
{
	...
	if (isp_hw->meta_viraddr) {
		dma_free_coherent(pdev, META_DATA_RAW_SIZE,
			(void *)isp_hw->meta_viraddr, (dma_addr_t)isp_hw->meta_phyaddr);
		isp_hw->meta_viraddr = 0;
		isp_hw->meta_phyaddr = 0;
	}
...
power_on_try:
	...
	cam_notice("%s isp poweron success. viraddr %p", __func__, isp_hw_data.viraddr);
	isp_hw_data.poll_start = true;
...
int hw_isp_poweroff(void)
{
	...
	flush_work(&isp_hw->meta_data_work);
...
void hw_isp_meta_data_ctrl(int ctrl, uint32_t attach_port)
{
...
inline int CHECK_PORT_VALID(int port)
{
...
void ispv3_hardware_set_ready(int port)
{
...
void ispv3_hardware_clear_ready(int port)
{
	if (CHECK_PORT_VALID(port) != 0) {
		cam_err(" invalid port %d",port);
		return;
...
int ispv3_hardware_update_addr(isp_port_e port, hwisp_buf_t *buf)
{
	...
	if (buf->info.port.use_phy_memory == 0){
		...
		cam_debug("%s iova=0x%lx.", __func__, buf->info.y_addr_iommu);

		/*config mmu registers*/
		ptba = hisi_reserved_smmu_phymem;
        ...
        /* use PHY addr*/
        //cam_debug("=====================phyaddr, port:%d", port);
        cam_debug("output  phyAddr=0x%lx.",  buf->info.y_addr_phy);
...
static int isp_write_overflow_handler(isp_port_e port)
{
	...
	cam_debug("overflow port =%d",port);
...
static void isp_update_output_addr(int index, hwisp_stream_buf_info_t *output_buf)
{
	...
	cam_debug("%s output iova=0x%lx. port = %d", __func__, output_buf->y_addr_iommu, output_buf->port.id);
	cam_debug("%s output uvoffset->u=0x%lx.", __func__, output_buf->u_addr_iommu);
	cam_debug("%s output uvoffset->v=0x%lx.", __func__, output_buf->v_addr_iommu);
...
static void ispv3_update_input_addr(hwisp_stream_buf_info_t *input_buf)
{
	...
	cam_debug("%s input iova=0x%lx. port = %d", __func__, input_buf->y_addr_iommu, input_buf->port.id);
	cam_debug("%s input uvoffset->u=0x%lx.", __func__, input_buf->u_addr_iommu);
	cam_debug("%s input uvoffset->v=0x%lx.", __func__, input_buf->v_addr_iommu);
...
static void  hw_isr_do_tasklet(unsigned long data)
{
    ...
    if (mac1_s0 & INT_ISP_MAC1_OVERFLOW0) {
        cam_debug("overflow port =%d",ISP_PORT_WRITE0);
    }
