...
int misp_load_fw(u8 *out_fw_disp)
{
	...
	if (drv_data->state != MINI_ISP_STATE_READY) {
		...
		if(!dsm_client_ocuppy(drv_data->client_extisp)){
			...
			dsm_client_notify(drv_data->client_extisp, DSM_EXTISP_ERROR_NO);
		}
	} else {
		cam_info("%s sucess to load fw.", __func__);
		if (0 == strncmp(drv_data->fw_info.info, "Pine", 4)) {
			cam_info("%s set sensor module id(%d %d %d).", __func__,
					s_module_id[0], s_module_id[1], s_module_id[2]);
			misp_send_cmd(drv_data, ISPCMD_CAMERA_SET_SENSOR_MODULE_ID,
					s_module_id, sizeof(s_module_id));
		}
		memcpy(out_fw_disp, drv_data->fw_info.info,
				sizeof(drv_data->fw_info.info));
	}
...
out:
	...
	if (!err) {
		for (index=0; index<MINI_ISP_MAX_ERR_CODE; index++) {
			...
			iSize = snprintf(tmpErr, sizeof(tmpErr), "[%d]: 0x%08x\n", index, drv_data->last_error_code[index]);
			strncat(errInfo, tmpErr, (sizeof(errInfo)-iSize));
		}
		if(!dsm_client_ocuppy(drv_data->client_extisp)){
			dsm_client_record(drv_data->client_extisp, "misp last error code:\n");
			dsm_client_record(drv_data->client_extisp, errInfo);
			dsm_client_notify(drv_data->client_extisp, DSM_EXTISP_ERROR_NO);
...
static void misp_dump_log(struct work_struct *work)
{
	...
	*pblock= SPI_BLOCK_BUF_SIZE;
	...
	set_fs(KERNEL_DS);
	fp = filp_open("/data/camera/misp.log", O_CREAT|O_APPEND|O_WRONLY, 0666);
	...
	if (fp->f_pos < (4*1024*1024L)) {
		...
		filp_close(fp, NULL);
		fp = filp_open("/data/camera/misp.log", O_CREAT|O_TRUNC|O_WRONLY, 0666);
		if (IS_ERR_OR_NULL(fp)) {
								...
								u32* reg_table, u32 reg_table_size)
{
	...
	snprintf(name, sizeof(name), "/data/camera/misp_0x%08x.regx",
													reg_table[0]);
				...
				struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (0 == strncmp("spi_config", pos, strlen("spi_config"))) {
		...
		misp_test_spi(pdata);
	}else if (0 == strncmp("test_dump_err", pos, strlen("test_spi"))) {
