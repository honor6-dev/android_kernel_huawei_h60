...
struct tps61310_private_data_t {
...
static struct tps61310_private_data_t tps61310_pdata;
static struct hisi_flash_ctrl_t tps61310_ctrl;
static struct i2c_driver tps61310_i2c_driver;

DEFINE_HISI_FLASH_MUTEX(tps61310);

/* Function define */
static int tps61310_set_strobe0(struct hisi_flash_ctrl_t *flash_ctrl,
	unsigned int state)
{
...
static int tps61310_set_reset(struct hisi_flash_ctrl_t *flash_ctrl,
	unsigned int state)
{
	struct tps61310_private_data_t *pdata;
...
static int tps61310_led_enable(struct hisi_flash_ctrl_t *flash_ctrl,
...
static int tps61310_get_dt_data(struct hisi_flash_ctrl_t *flash_ctrl)
{
	struct tps61310_private_data_t *pdata;
	...
	rc = of_property_read_u32_array(of_node, "hisi,flash-pin",
		pdata->pin, MAX_PIN);
	if (rc < 0) {
	...
	rc = of_property_read_u32(of_node, "hisi,flash_led13_num",
	...
	rc = of_property_read_u32(of_node, "hisi,flash_led2_num",
	...
	rc = of_property_read_u32(of_node, "hisi,torch_led13_num",
	...
	rc = of_property_read_u32(of_node, "hisi,torch_led2_num",
	...
	rc = of_property_read_u32_array(of_node, "hisi,flash_led2",
	...
	rc = of_property_read_u32_array(of_node, "hisi,flash_led13",
	...
	rc = of_property_read_u32_array(of_node, "hisi,torch_led2",
	...
	rc = of_property_read_u32_array(of_node, "hisi,torch_led13",
...
static ssize_t tps61310_lightness_show(struct device *dev,
...
static int tps61310_param_check(char *buf, unsigned long *param,
	int num_of_par)
{
	char *token;
	int base, cnt;
...
static ssize_t tps61310_lightness_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rc = tps61310_param_check((char *)buf, param, 2);
	if (rc < 0) {
	...
	if (cdata.mode == STANDBY_MODE) {
		...
		rc = tps61310_on(&tps61310_ctrl, &cdata);
		if (rc < 0) {
...
static ssize_t tps61310_flash_mask_show(struct device *dev,
	struct device_attribute *attr,char *buf)
{
        int rc=0;

        snprintf(buf, MAX_ATTRIBUTE_BUFFER_SIZE, "flash_mask_disabled=%d.\n",
		tps61310_ctrl.flash_mask_enable);
        rc = strlen(buf)+1;
        return rc;
}

static ssize_t tps61310_flash_mask_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	if ('0' == buf[0]) {
		tps61310_ctrl.flash_mask_enable = false;
	} else {
		tps61310_ctrl.flash_mask_enable = true;
	}
...
static void tps61310_torch_brightness_set(struct led_classdev *cdev,
	enum led_brightness brightness)
{
	...
	if (STANDBY_MODE == (flash_mode)brightness) {
		...
		/******************************************************
		  ...
		  ******************************************************/
		int max_level;
		max_level =  ((struct tps61310_private_data_t *)(tps61310_ctrl.pdata))->torch_led13_num;
...
static struct device_attribute tps61310_lightness =
    __ATTR(lightness, 0664, tps61310_lightness_show, tps61310_lightness_store);

static struct device_attribute tps61310_flash_mask =
    __ATTR(flash_mask, 0664, tps61310_flash_mask_show, tps61310_flash_mask_store);

static int tps61310_register_attribute(struct hisi_flash_ctrl_t *flash_ctrl, struct device *dev)
{
	int rc = 0;
	...
	flash_ctrl->cdev_torch.max_brightness
		= ((struct tps61310_private_data_t *)(flash_ctrl->pdata))->torch_led13_num
			*(TORCH_RIGHT_MODE-TORCH_MODE+1);
	flash_ctrl->cdev_torch.brightness_set = tps61310_torch_brightness_set;
	...
	rc = device_create_file(dev, &tps61310_flash_mask);
	if (rc < 0) {
...
static const struct of_device_id tps61310_dt_match[] = {
	{.compatible = "hisi,tps61310"},
	{}
};
MODULE_DEVICE_TABLE(of, tps61310_dt_match);
...
static struct i2c_driver tps61310_i2c_driver = {
	...
	.driver = {
		.name = "tps61310",
		.of_match_table = tps61310_dt_match,
	},
};
...
static struct hisi_flash_i2c_client tps61310_i2c_client;

static struct hisi_flash_fn_t tps61310_func_tbl = {
	.flash_config = hisi_flash_config,
	.flash_init = tps61310_init,
	.flash_exit = tps61310_exit,
	.flash_on = tps61310_on,
...
static struct hisi_flash_ctrl_t tps61310_ctrl = {
	...
	.hisi_flash_mutex = &flash_mut_tps61310,
	.pdata = (void*)&tps61310_pdata,
